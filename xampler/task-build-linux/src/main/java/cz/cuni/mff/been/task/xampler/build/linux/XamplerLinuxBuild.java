...
public class XamplerLinuxBuild extends Job {
	...
	protected void run() throws TaskException {
		...
		try {
			Untar.untar(xamplerSrcArchive.getAbsolutePath(), xamplerSrcRoot.getAbsolutePath(), "gzip");  
		} catch (AntTaskException e) {
		...
		if (!simulate) {
			...
			if (!buildFailed) logInfo("Build script returned successfully.");
		...
		if (!buildFailed) {
			try {
				Tar.tar(xamplerBinRoot.getAbsolutePath(),xamplerBinArchive.getAbsolutePath(),"gzip");
			} catch (AntTaskException e) {
