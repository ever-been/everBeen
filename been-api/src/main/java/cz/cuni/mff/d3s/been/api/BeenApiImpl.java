...
public class BeenApiImpl implements BeenApi {
	...
	public Collection<ServiceLogMessage> getServiceLogsByBeenId(final String participantId) throws BeenApiException {
		final String errorMsg = String.format(
				"Failed to list service logs for service wwith participant id '%s'",
				participantId);
	...
	// TODO @radek-macha, get rid of native query
			public
			Collection<ServiceLogMessage>
			getServiceLogsByDate(final Date date) throws BeenApiException {
		final String errorMsg = String.format(
		...
		final Query query = new QueryBuilder().nativa(String.format(
				"function() { var start = %d; var end = %d; return db.getCollection('%s.%s').find(" + "{created: {$gte: start, $lt: end}}, {_id: 0}).toArray(); }",
				timeToday,
				timeTomorrow,
				entityID.getKind(),
				entityID.getGroup()));
