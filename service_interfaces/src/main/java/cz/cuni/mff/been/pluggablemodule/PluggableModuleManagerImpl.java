...
public class PluggableModuleManagerImpl implements PluggableModuleManager {
	...
	/**
	 * Creates a new pluggable module manager for a hostruntime. Retrieved modules
	 * will be stored in given path. Only one instance of pluggable module manager
	 * should be created in an aplication.
	 * 
	 * @param tasksPortInterface
	 *          hostruntime
	 * @param pluggableModulesPath
	 *          path where to store extracted module packages.
	 */
	...
	*/
	 ...
	 * adds its jars to system classpath and instatiates an appropriate class that
	 * extends pluggable module. Method also resolves dependencies by recursively
	 * calling itself. For detecting circular dependencies,
	 * <code>loadingSet</code> structure is used.
	 * 
	 * @param moduleDescriptor
	 *          descriptor of module to load
	 * @param loadingSet
	 *          set of pluggable descriptors which are waiting for its
	 *          prerequisites.
	 * @return instance of requested pluggable module
	 * @throws cz.cuni.mff.been.pluggablemodule.PluggableModuleException
	 */
	private PluggableModule loadModule(
			PluggableModuleDescriptor moduleDescriptor,
			Set<PluggableModuleDescriptor> loadingSet) throws PluggableModuleException {
			...
			if (dependencies.isSetDependency()) {
				for (Dependency dependency : dependencies.getDependency()) {
					...
					if (registry.isRegistered(descriptor)) {
						...
						logger.info("Module \"" + dependency.getModuleName() + "\" will be loaded as a prerequisite.");
	...
	/**
	 * Extends system classpath by items specified in module's configuration file.
	 * 
	 * @param moduleDescriptor
	 *          pluggable module descriptor
	 * @param config
	...
	private Class<PluggableModule> loadModulesClasses(
			PluggableModuleDescriptor moduleDescriptor,
			PluggableModuleConfiguration config) throws PluggableModuleException {
			...
			if (java.isSetClasspathItems()) {
				...
				if (classPathItems.isSetClasspathItem()) {
					for (String filename : classPathItems.getClasspathItem()) {
						...
						if (!file.exists()) {
							throw new PluggableModuleException("Tried to add nonexistent file to system classpath.");
						}
						loadJarFile(file);
					}
				}
	 ...
	 * 
	 * @param moduleClass
	 *          module main class' class object
	...
	/**
	 * Returns a directory path where module with descriptor name should be
	 * stored.
	 * 
	 * @param moduleDescriptor
	 *          module's descriptor
	 ...
	 *          directory to create
	 * @throws IOException
	 *           if the directory does not exist and can not be created
	 */
	 ...
	 *           when something goes wrong.
	 */
	private PluggableModuleConfiguration readModuleConfiguration(
			PluggableModuleDescriptor moduleDescriptor) throws PluggableModuleException {
