...
/**
 * Class which represents version of software. Version is stored as triplet of
 * numbers and some free form string.<br>
 * <br>
 * Version should contain at least one version number (major version) but at
 * most 3 version numbers (major, minor and release numbers) and can contain
 * free form string with more precise description ("beta", "rc3", etc.). This
 * free-form string should be called "build identification string".<br>
 * Numbers in version string should be separated with "." (period) character.
 * Build identification string should be separated from the build numbers by "-"
 * (dash) or "_" (underscore) character. It should contain only non-capital
 * letters and numbers. Words in build id string should be separated by "-"
 * (dash).
 * 
 * Valid examples:<br>
 * <br>
 * 1.0.5-rc1 is represented as major=1, minor=0, release=5, buildID="-rc1"<br>
 * 1.0-20050404 is represented as major=1, minor=0, release=0,
 * buildID="-20050404"<br>
 * <br>
 * ValueVersion class is also able to handle "non-standard" version
 * representations. For example "1.0a" is represented as 1.0.0a (WITHOUT - or
 * _!).
 * 
 * @author Branislav Repcek
 * 
 */
public class ValueVersion implements ValueBasicInterface<ValueVersion>,
		Serializable, XMLSerializableInterface {

	private static final long serialVersionUID = -4558219241825107398L;

	/**
	 * Number of digits numerical fields will be aligned when converting to
	 * string.
	 */
	 ...
	 * 
	 * @param node
	 *            Node with version data.
	 * 
	 * @throws InputParseException
	 *             If there was an error while parsing input.
	 */
	...
	/**
	 * Create new <code>ValueVersion</code> with empty buil identification
	 * string.
	 * 
	 * @param majorVer
	 *            Major version number.
	 ...
	 * 
	 * @param versionString
	 *            String to convert to version value.
	 */
	...
	private void parseString(String versionString) {
		...
		if (strPos != 0) {
			major = Integer.valueOf(versionString.substring(0, strPos))
					.intValue();
		...
		if (strPos > strBeg) {
			minor = Integer.valueOf(versionString.substring(strBeg, strPos))
					.intValue();
		...
		if (strPos > strBeg) {
			release = Integer.valueOf(versionString.substring(strBeg, strPos))
					.intValue();
	 ...
	 * @param x
	 *            Value to test.
	 * @return Returns -1, 0, or 1 as given parameter is less than, equal to, or
	 *         greater than zero.
	 */
		...
		if (major > ver.major) {
			...
			if (minor > ver.minor) {
				...
				if (release > ver.release) {
					...
					return 4 * sgn(buildIdentifier
							.compareTo(ver.buildIdentifier));
				}
			}
		}
	}
	...
	/**
	 * Compare specified <code>ValueVersion</code> with this value for equality.
	 * Two versions are equal iff all their version numbers and build IDs are
	 * equal.
	 * 
	 * @param ver
	...
	/**
	 * Create textual representation of version. All numerical fields in the
	 * result will be aligned to <code>numericalAlignment</code> digits by
	 * zeroes from the left. Fields in version are separated by dot character.
	 * 
	...
	public String toStringAligned() {
		...
		if (buildIdentifier != null) {
			return align(major) + "." + align(minor) + "." + align(release)
					+ buildIdentifier;
		} else {
			return "(unknown)";
		}
	...
	public String toString() {
		...
		if (buildIdentifier != null) {
			return Integer.toString(major) + "." + Integer.toString(minor)
					+ "." + Integer.toString(release) + buildIdentifier;
		} else {
			return "(unknown)";
		}
	}
	...
	/**
	 * Convert number to string and fill with zeroes from right to create string
	 * with length of <code>numericalAlignment</code> characters.
	 * 
	 * @param val
	 *            Integer to convert.
	 * @return String with given number, result is at least
	 *         <code>numericalAlignment</code> characters long.
	 */
	...
	public void parseXMLNode(Node node) throws InputParseException {
		...
		if (!node.getNodeName().equals(getXMLNodeName())) {
			throw new InputParseException(
					"Node does not contain version data. Node name is \""
							+ node.getNodeName() + "\".");
		}
	...
	public Element exportAsElement(Document document) {
		 ...
		 * 
		 * where <version> is value of this class converted to the string using
		 * toString method.
		 */
	...
	/**
	 * @param buildId
	 *            New build identification string.
	 */
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *             If given release number is negative.
	 */
