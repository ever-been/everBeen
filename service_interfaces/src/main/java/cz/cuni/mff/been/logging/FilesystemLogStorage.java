...
public class FilesystemLogStorage implements LogStorage {
	...
	private class FileOutputHandle extends UnicastRemoteObject implements OutputHandle {
		 ...
		 */
		protected FileOutputHandle(String file) throws RemoteException, FileNotFoundException {
			super();
		...
		public void skipLines(long count) throws RemoteException, IOException {
			for (int i = 0; i < count; i++) {
				if (null == reader.readLine()) break;												// No reason to read past end.
			}
		}
	...
	/**
	 * Contains partial information about a log message, the information
	 * has to be parsed later.
	 * 
	 * @author Jaroslav Urban
	 */
	 ...
	 */
	public FilesystemLogStorage(String basedir)
	 ...
	 */
	public LogRecord[] getLogsForTask(String context, String taskID, long first, long last) throws LogStorageException, IllegalArgumentException, NullPointerException {
		checkTaskAndContextId(context, taskID);
	 ...
	 * @throws IOException
	 */
	private LogMessagePart parseLogMessage(String firstLine, BufferedReader reader)
	throws IOException {
	 ...
	 * @throws LogStorageException
	 */
	private LogRecord createLogRecord(LogMessagePart logPart,
			String context,
			String taskID,
			String hostname) 
	throws LogStorageException {
	 ...
	 * @throws LogStorageException
	 */
	private String getHostname(File taskDir)
	throws LogStorageException {
		File hostnameFile = new File(taskDir, HOSTNAME_FILE);
	...
	/*
	 * (non-Javadoc)
	 * @see cz.cuni.mff.been.logging.LogStorage#removeTask(java.lang.String, java.lang.String)
	 */
	@Override
	public void removeTask(String context, String taskId)
		...
		try {
			...
			throw new LogStorageException("Cannot delete the directory of the task: " 
					+ taskDir.getAbsolutePath(),
					e);
		}
