...
/**
 * Class which represents list of values of the same type.
 * <code>ValueList</code> is limited only to members of simple types (classes
 * which implement <code>ValueBasicInterface</code> interface).
 * 
 * @param <T>
 *            Type of the elements of the list. This type has to extend
 *            ValueBasicInterface< T >.
 * 
 * @author Branislav Repcek
 */
public class ValueList<T extends ValueBasicInterface<T>> implements
	...
	/**
	 * Allocate {@code ValueList} which represents an empty list. This
	...
	/**
	 * Create new list form array of objects implementing
	 * <code>ValueBasicInterface</code>. Elements are stored in same order as
	 * they appear in <code>newValue</code> array.
	 * 
	 * @param newValue
	 *            List of objects.
	 */
	 ...
	 * 
	 * @param values
	 *            Collection containing values that will be included in the
	 *            list.
	 */
	 ...
	 * 
	 * @param node
	 *            Node containing list data.
	 * 
	 * @throws InputParseException
	 *             If there was an error while parsing XML data.
	 */
	...
	/**
	 * Add all elements from specified list. Duplicate values are not removed.
	 * Order of elements is not changed.
	 * 
	 * @param list
	 *            List of values to add to the current list.
	 */
	...
	/**
	 * Add all elements from the specified collection to the current list.
	 * Duplicate entries are allowed. Order of elements is not changed.
	 * 
	 * @param list
	 *            Collection of elements to add to current list.
	 */
	 ...
	 * @param val
	 *            Value to remove.
	 * @return <code>true</code> if specified value was present in the list,
	 ...
	 * @param vb
	 *            Element to test.
	 * @return <code>true</code> if element has been found in list,
	 *         <code>false</code> otherwise.
	 */
	@Override
	...
	/**
	 * Compare specified <code>ValueList</code> with this value for equality.
	 * Comparison is based only on elements contained within respective lists,
	 * it does not matter in which order elements are stored.
	 * 
	 * @param vc
	...
	public void parseXMLNode(Node node) throws InputParseException {
		...
		if (!node.getNodeName().equals(getXMLNodeName())) {
			throw new InputParseException(
					"Node does not contain list data. Node name is \""
							+ node.getNodeName() + "\".");
		}

		elementType = ValueType.forName(XMLHelper.getAttributeValueByName(
				"subtype",
				node));
		...
		try {
			...
			throw new InputParseException(
					"Error creating instance of the list element.",
					e);
		}
		...
		try {
			subnodes = XMLHelper.getChildNodesByName(
					((T) subtypeClass.newInstance()).getXMLNodeName(),
					node);
		} catch (Exception e) {
			throw new InputParseException(
					"Unable to create instance of the list element.",
					e);
		}
	...
	public Element exportAsElement(Document document) {
		 ...
		 * Resulting node
		 * 
		 * <list subtype="<subtype-name>"> <element-node 0> . . . <element-node
		 * n-1> </list>
		 * 
		 * where <subtype-name> is canonical name of the type of elements in the
		 * list or (none) if list is empty, <element-node 0> to <element-node
		 * n-1> are serialised items from the list. These nodes will not be
		 * present if the list is empty.
		 */

		Element element = document.createElement(getXMLNodeName());

		element.setAttribute("subtype", elementType == null ? "(none)"
				: elementType.NAME);
