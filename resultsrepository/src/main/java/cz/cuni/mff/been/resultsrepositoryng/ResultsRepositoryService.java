...
public class ResultsRepositoryService extends Service {
	...
	protected void start() throws TaskException {
		try {
			if (Debug.isDebugModeOn()) {
				derbyPluggableModule.startEngine(
						getDerbyHomeDir(),
						true,
						RR_DERBY_DEBUG_NETWORK_PORT);
			} else {
			...
			implementation = new ResultsRepositoryImplementation(
					...
					RR_DATABASE_NAME,
					CurrentTaskSingleton.getTaskHandle().getWorkingDirectory()
							+ File.separator + "filestore");
			...
			CurrentTaskSingleton.getTaskHandle().logError(
					"Couldn't initialize RR because error occured: "
							+ e.getMessage());
	...
	protected void stop() throws TaskException {
		...
		try {
			...
			CurrentTaskSingleton.getTaskHandle()
					.logError(
							"Couldn't stop RR because error occured: "
									+ e.getMessage());
	...
	private String getDerbyHomeDir() {
		return CurrentTaskSingleton.getTaskHandle().getWorkingDirectory();
	}
