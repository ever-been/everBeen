...
/**
 * This class holds <code>DataStructures</code> of <code>Task Manager</code>
 * and serves as an interface to it.
 * 
...
//	private final Rescue rescue;
	 ...
	 * Add new Task.
	 * 
	 * @param taskEntry <code>TaskEntry</code> for new Task.
	 * @throws NullPointerException If input parameter is <code>null</code>.
	 * @throws DataRuntimeException If internal error occurred.
	 */
	public synchronized void newTask(TaskEntry taskEntry, TaskData taskData) {
		...
		/*
		 * TODO
		 * AP: This cloning caused CLI to receive out-of-data 'submitted' state all the time.
		 * There is no point in cloning the task entry on input. OK, it should be cloned on
		 * output, but that definitely does happen on RMI transfers anyway. All those methods
		 * like getTaskBySomething() clone, but there seems to be no reason for that.
		 */
...
//		this.rescue.addTask(newTaskEntry, taskData);
	 ...
	 * Add new Context.
	 * 
	 * @param contextEntry <code>ContextEntry</code> for new Context.
	 * @throws NullPointerException If input parameter is <code>null</code>.
	 * @throws DataRuntimeException If internal error occurred.
	 */
...
//		this.rescue.addContext(newContextEntry);
	 ...
	 * Add new CheckPoint.
	 * 
	 * @param checkPointEntry <code>CheckPointEntry</code> for new CheckPoint.
	 * @throws NullPointerException If input parameter is <code>null</code>.
	 * @throws DataRuntimeException If internal error occurred.
	 */
...
//		this.rescue.addCheckPoint(newCheckPointEntry);
	 ...
	 * @throws NullPointerException If input parameter is <code>null</code>.
	 */
	public synchronized void newCheckPointOver(
...
//					checkPoint.getTaskId(), checkPoint.getContextId());
	...
	public synchronized void delTask(String taskId, String contextId) {
		...
		try {
			...
			/* There's nothing we can do here. The task may have been killed and deleted
			 * explicitely from the CLI or a race condition (automatic and manual context
			 * deletion) could occur. If the leaf doesn't exist, there's no need to worry. */
		} catch (RemoteException exception) {														// Local call. Should never happen.
...
//		this.rescue.remTask(taskId, contextId);
	 ...
	 * @param contextId ID of Context.
	 * @throws NullPointerException If input parameter is <code>null</code>.
	 * @throws IllegalArgumentException If context not found.
	 */
...
//		this.rescue.remContext(contextId);
	 ...
	 * 	<code>null</code>.
	 * @throws IllegalArgumentException If task not found.
	 * @throws IllegalStateException If taskInterface is already set.
	 */
...
//		this.rescue.changeTask(task, taskData);
	...
	/**
	 * Change state of specified task.
	 * Allowed changes:
	 * 	SUBMITTED : not allowed
	 * 	SCHEDULED : allowed only from SUBMITTED
	 * 	RUNNING : allowed only from SHEDULED or SLEEPING
	 * 	SLEEPING : allowed only from RUNNING
	 * 	FINISHED : allowed only from RUNNING
	 * 
	 * @param taskId ID of task.
	 * @param contextId ID of context.
	 * @param newState New state of task.
	 * @throws NullPointerException If input parameter is <code>null</code>.
	 * @throws IllegalArgumentException If task not found or change to required
...
//		this.rescue.changeTask(task, taskData);
	 ...
	 * 
	 * @param hostName Name of host running HostRuntime.
	 * @param newReservation ID of Context, <code>null</code> or
	 * 	<code>""</code>.
	 * @throws NullPointerException If input parameter is <code>null</code>.
	 * @throws IllegalArgumentException If hostRuntime not found or required
...
//		this.rescue.changeHostRuntime(hostRuntime);
	 ...
	 * @throws IllegalArgumentException If task not found.
	 */
	public synchronized void notifyTaskRestarted(
			String taskId, String contextId) {
		/* Check input parameters. */
		if (taskId == null) {
			throw new NullPointerException("taskId is null");
...
//		this.rescue.changeContext(context);
	 ...
	 * 
	 * @param taskEntries Array of <code>TaskEntry</code> objects to clone.
	 * @return Array of cloned <code>TaskEntry</code> objects.
	 * @throws DataRuntimeException If internal error occurred.
	 */
	private TaskEntry[] cloneTaskEntries(TaskEntry[] taskEntries) {
	 ...
	 * @return Array of cloned <code>TaskEntry</code> objects.
	 */
	private TaskEntry[] getTasks(String taskId, String contextId, String hostName) {
		TaskEntry[] taskEntries = this.data.getTasks(taskId, contextId, hostName);
	 ...
	 */
	public synchronized TaskEntry[] getTasks() {
	 ...
	 * @throws NullPointerException If input parameter is <code>null</code>.
	 */
	public synchronized TaskEntry[] getTasksInContext(String contextId) {
		if (contextId == null) throw new NullPointerException ("Attempt to get tasks in null context.");
	 ...
	 * @throws NullPointerException If input parameter is <code>null</code>.
	 */
	public synchronized TaskEntry[] getTasksOnHost(String hostName) {
		if (hostName == null) throw new NullPointerException ("Attempt to get tasks on null host.");
	 ...
	 * 
	 * @param taskState State of task.
	 * @return Array of <code>TaskEntry</code> objects.
	 * @throws NullPointerException If input paremeter is <code>null</code>.
	 */
	public synchronized TaskEntry[] getTasksByState(TaskState taskState) {
		/* Check input parameters. */
		if (taskState == null) {
	 ...
	 * @throws DataRuntimeException If internal error occurred.
	 */
	public synchronized TaskEntry getTaskById(
			String taskId, String contextId) {
		/* Check input parameters. */
		if (taskId == null) {
			throw new NullPointerException("taskId is null");
	 ...
	 * 
	 * @param contextEntries Array of <code>ContextEntry</code> objects to
	 * 	clone.
	 ...
	 * 
	 * @param checkPointEntries Array of <code>CheckPointEntry</code> objects
	 * 	to clone.
	...
	public synchronized CheckPointEntry getCheckPoint(
			String type, String value, String taskId, String contextId) {
...
//	 * 
//	 * @param hostRuntimeEntries Array of <code>HostRuntimeEntry</code> objects
//	 * 	to clone.
...
//	 * 
//	 * @param hostName Name of host.
//	 * @return Cloned <code>HostRuntimeEntry</code> object or <code>null</code>
//	 * 	if not found.
//	 */
...
//		this.rescue.addHostRuntime(hostRuntime);
	...
	throws IllegalArgumentException {
		 ...
		 * ABORTED) */
		TaskEntry[] tasks
			= this.data.getTasks(null, null, hostName);
