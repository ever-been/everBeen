...
/**
 * Implementation of file agents that stores files 
 * into a dataset in RR.
 * 
 * In this implementation, files are stored 
 * into RR file store and they are referenced
 * by UUID identified for RR's dataset.
...
public class RRFileAgentImpl implements FileAgent {
	 ...
	 */
	private void initResultsRepositoryReferences() throws RemoteException, PluggableModuleException {
		Object rr = Task.getTaskHandle().getTasksPort().serviceFind(
				ResultsRepositoryService.SERVICE_NAME,
				Service.RMI_MAIN_IFACE ); 
		if( rr == null ){
			throw new PluggableModuleException("Results Repository reference cannot be obtained. Maybe it is not running.");
		}
	 ...
	 * @throws PluggableModuleException
	 */
	public RRFileAgentImpl(String analysisName, String datasetName, String dataFieldName) throws PluggableModuleException {
		...
		try {
			...
			throw new PluggableModuleException("Error initializing RR reference.", e);
		}
	}
	
	@Override
	public boolean fileExists(Map<String, Serializable> tags) throws IOException {
		try {
	...
	public void loadFile(File file, Map<String, Serializable> tags)
		...
		try {
			UUID fileId = evaluateCondition(tags);
			if (fileId == null) {
				throw new IOException("Requested file not found (no record satisfies condition or file id set to null).");
			}
	...
	public void storeFile(File file, Map<String, Serializable> tags)
		...
		try {
			dd = resultsRepositoryManager.getDatasetDescriptor(analysisName, datasetName);
		} catch (ResultsRepositoryException e) {
		...
		for ( String name : tags.keySet()) {
			...
			if (type == null) {
				throw new IOException("Error storing file: Tag named \""+name+"\" does not exist.");
			}
	...
	private UUID evaluateCondition(Map<String, Serializable> tags) throws IOException, ResultsRepositoryException {
		...
		if (result.size() > 1) {
			throw new IOException("More files satisfy condition (try including all key tags into condition criteria).");
		}
