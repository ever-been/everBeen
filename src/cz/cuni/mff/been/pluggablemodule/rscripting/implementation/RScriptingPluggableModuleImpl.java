 ...
 */
public class RScriptingPluggableModuleImpl extends PluggableModule implements RScriptingPluggableModule {
	 ...
	 */
	public int runRScript(String[] script, String homeDirectory) throws ScriptException {
	 ...
	 * @throws ScriptException
	 */
	public int runRScript(File scriptFile, String homeDirectory) throws ScriptException {
		...
		try {
			...
			/* this needs to be closed otherwise JVM creates a pipe that will never be closed */
			p.getOutputStream().close();
			
			stdOutReader = new OutputReader(p.getInputStream(), OutputType.STDOUT);
			stdOutReader.start();
			
			stdErrReader = new OutputReader(p.getErrorStream(), OutputType.STDERR);
			stdErrReader.start();
