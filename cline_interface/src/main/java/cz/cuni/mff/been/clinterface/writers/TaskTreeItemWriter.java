...
public final class TaskTreeItemWriter extends AbstractWriter {
	 ...
	 * 
	 * @param query
	 *            The task tree interface to read from.
	 * @param address
	 ...
	 */
	public void
			sendLineFlagsSimple(TaskTreeQuery query, TaskTreeAddress address)
					throws IOException {
		...
		try {
			...
			CurrentTaskSingleton.getTaskHandle().logWarning(
					"Branch unavailable: "
							+ assemblePath(query.getPathAt(address)));
			return;
		}
		switch (record.getType()) {
			case LEAF:
				TaskEntry entry = record.getTask();
				builder().append("L ").append(generateName(record)).append(' ')
				...
				break;
			case NODE:
				builder().append("N ").append(generateName(record))
						.append(" |");
				sendFlags(record.getFlags());
				builder().append("|\n");
	 ...
	 */
	public void
			sendLinePlainSimple(TaskTreeQuery query, TaskTreeAddress address)
					throws IOException {
		TaskTreeRecord record;
	 ...
	 *            The address to start at.
	 * @param level
	 *            Tree depth (and text indentation) level.
	 * @throws IOException
	 *             When it rains.
	 */
	public void sendLineFlagsRecursive(
			TaskTreeQuery query,
		...
		switch (record.getType()) {
			case LEAF:
				...
				builder().append(generateName(record)).append(' ')
						.append(entry.getTaskId()).append(' ')
						.append(literal(entry.getState())).append(' ')
						.append(literal(entry.getHostName())).append(" |");
				sendFlags(record.getFlags());
	 ...
	 *             When it rains.
	 */
	public void sendLinePlainRecursive(
			TaskTreeQuery query,
			TaskTreeAddress address,
			int level) throws IOException {
		TaskTreeRecord record;
	...
	/**
	 * Outputs data from a task tree item with flag values. No recursion, no
	 * sendOut().
	 * 
	 * @param query
	 *            The task tree interface to read from.
	 ...
	 *             When it rains.
	 */
	private void sendLineFlagsSimple2(
			TaskTreeQuery query,
			TaskTreeAddress address) throws IOException {
		TaskTreeRecord record;
		...
		switch (record.getType()) {
			case LEAF:
				TaskEntry entry = record.getTask();
				builder().append("L \t").append(generateName(record))
						.append(' ').append(entry.getTaskId()).append(' ')
						.append(literal(entry.getState())).append(' ')
						.append(literal(entry.getHostName())).append(" |");
				sendFlags(record.getFlags());
				builder().append("|\n");
	 ...
	 *             When it rains.
	 */
	public void sendLinePlainSimple2(
			TaskTreeQuery query,
			TaskTreeAddress address) throws IOException {
		TaskTreeRecord record;
	...
	private void sendFlags(Pair<TaskTreeFlag, TreeFlagValue>[] flags) {
		...
		if (flags.length > 0) {
			...
			builder().append(literal(flag)).append('=')
			...
			while (i < flags.length) {
				flag = flags[i].getKey();
				value = flags[i++].getValue();
				builder().append(',').append(literal(flag)).append('=')
						.append(literal(value.getOrdinal())).append('-')
						.append(value.getOrdinal().ordinal()).append("-\"")
						.append(quotedLiteral(value.getMessage())).append('"');
			}
	 ...
	 * 
	 * @param record
	 *            A TaskTreeRecord with node information.
