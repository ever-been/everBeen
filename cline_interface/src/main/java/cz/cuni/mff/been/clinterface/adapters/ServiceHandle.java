...
public class ServiceHandle {
	...
	public enum Status {
		 ...
		 * 
		 * @param name Human-readable name of the status.
		 */
		 ...
		 * 
		 * @param s Human-readable name of the status.
	 ...
	 * Unfortunately, it's in a different JVM and there's no way to notify the interface. 
	 */
	 ...
	 * @throws CommandLineException When something goes wrong.
	 */
	public void acquireData( TaskManagerInterface taskManager ) throws ModuleSpecificException {
		long newTime;
	...
	/**
	 * Reset acquire time so that data is reacquired on all subsequent operations.
	 */
	 ...
	 * 
	 * @param taskManager A reference to the Task Manager.
	 * @param host The host on which the task should run.
	 * @param debug Whether to start in debugging mode.
	 * @throws ModuleSpecificException When input something goes wrong.
	 */
	@SuppressWarnings( "deprecation" )
	public synchronized void start( TaskManagerInterface taskManager, String host, boolean debug )
	throws ModuleSpecificException {
		...
		if ( null == status ) {
			...
			taskDescriptor = TaskDescriptorHelper.createBootTask( bootTask, host );
			...
			try {
				for ( counter = 0; counter < 50; ++counter ) {
					try {
						...
						Task.getTaskHandle().logWarning( "BUG in TM! Delayed service unlisting." );
						try {
				...
				if ( 50 == counter ) {
					Task.getTaskHandle().logFatal( "TASK MANAGER SUX!!!" );
					throw new ModuleSpecificException( Errors.CONN_TM_TIMEOUT, " (" + name + ')' );
				}
				
				forceAcquireData( taskManager, new Date().getTime() );
				while (																				// TODO: Remove active waiting!
					status != Status.RUNNING ||
					null == taskManager.serviceFind( name, Service.RMI_CONTROL_IFACE )
				) {
					try {
						Thread.sleep( SHORT_TIMEOUT );
					} catch ( InterruptedException e ) {}
				...
				throw new ModuleSpecificException( Errors.CONN_TM, " (" + name + ')', exception );
			}
			
			// MODULE.sendEvent( Event.SERVICE_STATUS_CHANGE );										// Calls invalidate(), too.
		} else {
	 ...
	 * 
	 * @param taskManager A reference to the Task Manager.
	 * @throws ModuleSpecificException When something goes wrong.
	 */
	...
	throws ModuleSpecificException {
		...
		if ( status != Status.RUNNING ) {
			...
			try {
				controlInterface = (ServiceControlInterface)
				taskManager.serviceFind( name, Service.RMI_CONTROL_IFACE );
			} catch ( RemoteException exception ) {
				throw new ModuleSpecificException(
					Errors.CONN_TM_LOOKUP,
					" (" + name + ')',
					exception
				);
			}
			...
			if ( null == controlInterface ) {
				throw new ModuleSpecificException( Errors.CONN_TM_REG, " (" + name + ')' );
			}
			...
			try {
				...
				while (																				// TODO: Remove active waiting!
					status != null ||
					taskManager.serviceFind( name, Service.RMI_CONTROL_IFACE ) != null
				) {
					try {
						Thread.sleep( SHORT_TIMEOUT );
					} catch ( InterruptedException e ) {}
			...
			// MODULE.sendEvent( Event.SERVICE_STATUS_CHANGE );										// Would be nice.
		}
	}
	 ...
	 * 
	 * @param taskManager A reference to the Task Manager.
	 * @param debug Whether to restart the task in debugging mode.
	 * @throws ModuleSpecificException
	 */
	public synchronized void restart( TaskManagerInterface taskManager, boolean debug )
	throws ModuleSpecificException {
		acquireData( taskManager );
		if ( status != Status.RUNNING ) {
			throw new ModuleSpecificException( Errors.STAT_STOPPED, " (" + name + ')' );
	 ...
	 * 
	 * @param taskManager A reference to the Task Manager.
	 * @param from First log record to extract.
	 * @param to Last log record to extract.
	 * @return An array of log records.
	 * @throws ModuleSpecificException When something goes wrong.
	 */
	public LogRecord[] getLogs( TaskManagerInterface taskManager, long from, long to )
	throws ModuleSpecificException {
		try {
			...
			throw new ModuleSpecificException( Errors.CONN_LOG_LOG, " (" + name + ')', exception );
		} catch ( LogStorageException exception ) {
			throw new ModuleSpecificException( Errors.CONN_LOG_LOG, " (" + name + ')', exception );
		} catch ( IllegalArgumentException exception ) {
			throw new ModuleSpecificException( Errors.STAT_NOTYET, " (" + name + ')', exception );
		}
	}
	...
	throws ModuleSpecificException {
		try {
			return taskManager.getStandardOutput( TaskManagerInterface.SYSTEM_CONTEXT_ID, tid );
		} catch ( RemoteException exception ) {
	...
	throws ModuleSpecificException {
		try {
			return taskManager.getErrorOutput( TaskManagerInterface.SYSTEM_CONTEXT_ID, tid );
		} catch ( RemoteException exception ) {
			throw new ModuleSpecificException( Errors.CONN_TM, exception );
		} catch ( LogStorageException exception ) {
			throw new ModuleSpecificException(
	...
	public void receiveEvent( Event event ) {
		if ( event == Event.SERVICE_STATUS_CHANGE ) {
			invalidate();
		}
	}
	*/
	 ...
	 * 
	 * @param taskManager A remote reference pointing at the Task Manager.
	 * @throws CommandLineException When something bad happens.
	 */
	private void forceAcquireData( TaskManagerInterface taskManager, Long newTime )
	throws ModuleSpecificException {
		URI uri;
		...
		try {
			...
			status = Status.fromString(
				(String) taskManager.checkPointLook(
			...
			Task.getTaskHandle().logFatal( "Invalid service URL!" );
			status = null;
