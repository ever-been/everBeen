...
/**
 * A command line interface component that corresponds to the packages listing
 * screen.
 * 
...
public final class PackagesModule extends CommandLineModule {
	...
	/**
	 * Active waiting delay. Active waiting is needed to overcome design flaws
	 * of SW Repository.
	 */
	 ...
	 * @author Andrej Podzimek
	 */
	private static final class MatchRegex implements
	 ...
	 * @author Andrej Podzimek
	 */
	private static final class MatchEverything implements
			PackageQueryCallbackInterface {
	...
	static {
		...
		actions.put("list", new CommandLineAction<PackagesModule>() {
			...
			public void handle(
				...
				if (null == typeNames) {
					...
					types = new boolean[PackageType.values().length]; // Initialized
																		// to
																		// false.
				...
				try {
					softwareRepository = module.softwareRepositoryReference
							.get();
					if (null == pattern) {
						if (null == types) {
							if (request.getFlag("desc")) {
								for (PackageMetadata metadata : softwareRepository
										.queryPackages(MATCH_EVERYTHING)) {
						...
						if (null == types) {
							if (request.getFlag("desc")) {
								for (PackageMetadata metadata : softwareRepository
										.queryPackages(new MatchRegex(cpattern))) {
									writer.sendLineXtend(metadata);
								}
							} else {
								for (PackageMetadata metadata : softwareRepository
		...
		actions.put("upload", new CommandLineAction<PackagesModule>() {
			...
			public void handle(
				...
				try {
					transporter = new InputStreamTransporter(request
							.getBlobStream(), monitor);
				} catch (RemoteException exception) {
					throw new ModuleSpecificException(Errors.CONN_SR, exception);
				}
				...
				try {
					synchronized (monitor) {
						handle = softwareRepository
								.beginPackageUpload(transporter);
						for (;;) {
							try {
								monitor.wait();
								break;
					...
					for (boolean sleepOn = true; sleepOn;) {
						switch (softwareRepository
								.getPackageUploadStatus(handle)) {
							case ACCEPTED:
								sleepOn = false;
								break;
							case ERROR:
							...
							case REJECTED:
								...
								for (String message : softwareRepository
										.getUploadErrorMessages(handle)) {
									errors.append('\n') // Initial '\n' will be
														// discarded.
											.append(message);
								}
		...
		actions.put("download", new CommandLineAction<PackagesModule>() {
			...
			public void handle(
				...
				try {
					transporter = new OutputStreamTransporter(response
							.getStandardOutputStream(), monitor);
				} catch (RemoteException exception) {
					throw new ModuleSpecificException(Errors.CONN_SR, exception);
				}
				...
				try {
					synchronized (monitor) { // Condition Variable semantics...
						...
						for (;;) {
							try {
								monitor.wait(); // Notified when the stream
												// ends.
								break;
							} catch (InterruptedException exception) {
							}
					...
					for (boolean sleepOn = true; sleepOn;) {
						switch (softwareRepository
								.getPackageDownloadStatus(handle)) {
							case SUCCEEDED:
								sleepOn = false;
								break;
							case ERROR:
							...
							default:
								...
								CurrentTaskSingleton.getTaskHandle().logInfo(
										"Had to wait for download.");
						}
					}
					softwareRepository.endPackageDownload(handle); // Release
																	// the
																	// resources.
				} catch (RemoteException exception) {
					throw new ModuleSpecificException(Errors.CONN_SR, exception);
		...
		actions.put("delete", new CommandLineAction<PackagesModule>() {
			private final Set<String> parameters = stringSet("filename");
			private final Set<String> flags = stringSet();
			private final String help = constructHelp(
					parameters,
			...
			public void handle(
				...
				try {
					if (!module.softwareRepositoryReference.get()
		...
		actions.put("help", new CommandLineAction<PackagesModule>() {
	...
	private final ServiceReference<SoftwareRepositoryInterface> softwareRepositoryReference;
