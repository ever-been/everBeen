...
/**
 * This class represents a tree of task descriptors. This tree has nothing in
 * common with the order of task execution or the method of results collection.
 * It used <b>only</b> on the web interface to simplify the task list
 * presentation.
 * 
 * @author Andrej Podzimek
 */
public final class TaskTree extends UnicastRemoteObject implements
		TaskTreeInput, TaskTreeQuery {
	...
	/**
	 * A shared lock acquired when tree data is read or modified (without
	 * structural changes).
	 */
	...
	private final HashMap<String, TaskTreeAddressBody> pathToAddress = // Protected
																		// by
																		// address
																		// locks.
	...
	private final HashMap<Long, TaskTreeAddressBody> hashToAddress = // Protected
																		// by
																		// address
																		// locks.
	...
	private final HashMap<Long, TaskTreeLeaf> hashToLeaf = // Protected by tree
															// locks.
	new HashMap<Long, TaskTreeLeaf>();

	/** Special mapping of hash codes to tree nodes. Just to avoid casting. */
	private final HashMap<Long, TaskTreeNode> hashToNode = // Protected by tree
															// locks.
	...
	/**
	 * The default constructor. Initializes a new empty task tree with a root
	 * node.
	 */
	public TaskTree() throws RemoteException {
		...
		this.pathToAddress.put(
				this.rootAddress.getPathString(),
				this.rootAddress);
	...
	public void addLeaf(TaskTreeAddress address, TaskEntry entry)
		...
		try {
			...
			if (null == type) { // No, we don't know the address.
				...
				stack = new ArrayDeque<TaskTreeAddressBody>(); // Stack of
																// unknown
																// addresses.
				do {
					stack.push(curAddr); // Store the unknown address.
					curAddr = getParentAddress(curAddr); // Get its parent.
					curHash = curAddr.longHashCode(); // Compute the parent's
														// hash.
				} while (!hashToType.containsKey(curHash)); // Do we already
															// know this parent?
				curNode = hashToNode.get(curHash); // Get the first known
													// ancestor!
				if (null == curNode) { // Exists && !Node => Leaf
					throw new IllegalAddressException( // Leaves can't have
														// children.
				...
				while (stack.size() > 1) { // For all our unknown ancestors:
					...
					curNode = new TaskTreeNode(this, curAddr, curParentAddr); // Create
																				// the
																				// new
																				// node.
					curParentNode.addChild(curAddr); // Let the parent know.
					hashToType.put(curHash, Type.NODE); // Register node's type.
					hashToNode.put(curHash, curNode); // Register node's
														// instance.
				}
				...
				curLeaf = new TaskTreeLeaf(this, curAddr, curParentAddr, entry); // Create
																					// the
																					// new
																					// leaf.
				...
				switch (type) {
					...
					case LEAF: // A leaf can be reinserted...
						if (entry != hashToLeaf.get(curHash).getTask()) { // ...if
																			// it
																			// carries
																			// the
																			// same
						...
						curParentAddr = getParentAddress(curAddr); // Just find
																	// the
																	// parent
																	// address...
						hashToNode.get(curParentAddr.longHashCode()).addChild(
								curAddr); // ...and re-add the child.
						break;
					default:
						throw new IllegalStateException(
								"The Impossible has happened"); // This should
																// never happen.
				}
			}
		} finally {
	...
	@Override
	public void addNode(TaskTreeAddress address)
			throws IllegalAddressException, RemoteException {
		Type type;
		Long curHash;
		TaskTreeAddressBody firstAddr, curAddr, curParentAddr;
	...
	public TaskTreeAddress[] getChildrenAt(TaskTreeAddress address)
		...
		try {
			...
			switch (type) {
				case LEAF:
					return addresses(hashToLeaf.get(hash).getChildren()); // Yes,
																			// this
																			// throws.
																			// OK.
				case NODE:
	...
	public TaskEntry getTaskAt(TaskTreeAddress address)
		...
		try {
			...
			switch (type) {
				...
				case NODE:
					return hashToNode.get(hash).getTask(); // Yes, this throws.
															// OK.
				default:
					throw new IllegalStateException("Unknown enum member.");
			}
	...
	public String[] getPathAt(TaskTreeAddress address) {
		...
		try {
			return addressFromHash(address.longHashCode()).getPathSegments(); // It
																				// must
																				// exist.
		} finally {
	...
	public void setFlag(
			...
			TaskTreeFlag flag,
			Enum<?> ordinal) throws IllegalAddressException, TreeFlagException,
			RemoteException {
	...
	public void setFlag(
			...
			TaskTreeFlag flag,
			Enum<?> ordinal,
			Serializable message) throws IllegalAddressException,
			TreeFlagException, RemoteException {
		flag.validate(ordinal);
		treeReadLock.lock();
		try {
			setFlagNonblocking(address.longHashCode(), flag, new TreeFlagValue(
					ordinal,
					message));
		} finally {
			treeReadLock.unlock();
	...
	@Override
	public TreeFlagValue getFlagValueAt(
			TaskTreeAddress address,
			TaskTreeFlag flag) throws RemoteException, IllegalAddressException {
		treeReadLock.lock();
		try {
	...
	public TaskTreeRecord getRecordAt(
			...
			boolean path,
			boolean data,
			boolean flags) throws IllegalAddressException, RemoteException {
		TaskTreeAddressBody addr;
		Long hash;
		Type type;
		TaskTreeBasic el;
	...
	/**
	 * Implementation of he flag getter that does not care about node locking.
	 * (Called from places where locks are already acquired. For example, all
	 * the needed locks are guaranteed to be acquired when inheritance handlers
	 * run.)
	 * 
	 * @param hash
	 *            Hash code of the address pointing at the requested node.
	 * @param flag
	 *            The flag to return.
	 * @return A flag value instance from the required flag.
	 * @throws IllegalAddressException
	 *             When an invalid address is supplied.
	 */
	...
	/**
	 * Implemetation of the flag setter that does not care about node locking.
	 * (Called from places where locks are already acquired.
	 * 
	 * @param hash
	 *            Hash code of the address pointing at the node in which the
	 *            flag will beset.
	 * @param flag
	 *            The flag to modify.
	 * @param newValue
	 ...
	 *             When troubles with flags occur, possibly due to bugs. ;-)
	...
	private void setFlagNonblocking(
			Long hash,
			TaskTreeFlag flag,
			TreeFlagValue newValue) throws IllegalAddressException,
			TreeFlagException, RemoteException {
		TaskTreeAddressBody addr;
		...
		try {
			...
			while (addr != rootAddress) {
				...
				if (!flag.inherit(oldValue, newValue, inheritance)) {
					return;
				}
	...
	/*
	 * Automatic flags cleanup should not occur on this level. The user and
	 * maintainer of the given task should follow the inheritance rules of the
	 * flags he uses and reset flags to default values when necessary.
	 */
	...
	/**
	 * A special parent address getter. Tries to look up the address in the
	 * database. Creates a new one (efficiently) if necessary.
	 * 
	 * @param address
	 *            An address to which the parent address should be computed.
	...
	private TaskTreeAddressBody getParentAddress(TaskTreeAddressBody address)
		...
		synchronized (hashToAddress) {
			...
			if (null == result) {
				result = new TaskTreeAddressBody(
						parentPath,
						address.parentSegments());
				while (hashToAddress.containsKey(result.longHashCode())) {
					result.rehash();
				}
	...
	/**
	 * Scans a subtree using DFS and removes all of its nodes from this tree's
	 * tables. When this method is called, the root of the deleted node must be
	 * still present at least in hashToItem. hashToType and hashToNode need not
	 * contain that element any more. However, it should not be deleted in
	 * advance for preformance reasons. (The mappings need to be accessed
	 * anyway.)
	 * 
	 * @param root
	 *            Address of the root element to remove.
	 * @throws RemoteException
	 *             Almost never. Should manipulate local objects only!
	 */
	private void clearNodeRecursive(TaskTreeAddressBody root)
			throws RemoteException {
		Long hash;
		Type type;
		...
		switch (type) {
			case NODE:
				...
				break;
			case LEAF:
				// clearFlagsNonblockingMustExist( root, hashToLeaf.remove( hash
				// ).getFlags() );
				/*
				 * Unsetting flags should be the user's responsibility. The user
				 * knows how the corresponding flag's inheritance is designed
				 * and whether flags of deleted items need to be reset or not.
				 */
	...
	/**
	 * Converts an array of internal task tree adress representation intsances
	 * to an array of the publically visible hash codes.
	 * 
	 * @param addresses
	 *            The collection of addresses to convert.
	 * @return An array of publically visible task tree addresses.
	 */
	private TaskTreeAddress[] addresses(
			Collection<TaskTreeAddressBody> addresses) {
