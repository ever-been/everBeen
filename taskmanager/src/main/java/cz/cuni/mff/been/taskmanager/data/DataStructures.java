 ...
 * 
 * Note: I added some quick fixes - that means ugly hacks - to make stupid
 * exceptions go away. This thing was crashing practically every time with null
 * pointer exception on shutdown... All (I think) such ugly hacks have been
 * marked with comments (XXX).
 * 
 * BR
 */
...
public class DataStructures {
	 ...
	 */
	private final LinkedList<TaskNode> tasks;

	/**
	 * List of all Contexts in this <code>DataStructures</code> system.
	 */
	private final LinkedList<ContextNode> contexts;

	/**
	 * List of all Host Runtimes in this <code>DataStructures</code> system.
	 */
	private final LinkedList<HostRuntimeNode> hostRuntimes;
	...
	/**
	 * Return <code>TaskNode</code> associated with Task specified by its taskId
	 * and contextId.
	 * 
	 * @param taskId
	 *          ID of Task.
	 ...
	 */
	private TaskNode findTask(String taskId, String contextId) {
		TaskNode task = null;
	...
	protected synchronized TaskEntryImplementation[] getTasks(String taskId,
		...
		if (contextId != null) {
			...
			if (contextNode == null) {
				return new TaskEntryImplementation[0];
			}
		...
		LinkedList<TaskEntryImplementation> foundTaskEntries = new LinkedList<TaskEntryImplementation>();

		/* Find required entries. */
		while (taskIterator.hasNext()) {
			TaskNode taskNode = taskIterator.next();
			TaskEntryImplementation taskEntry = taskNode.getTaskEntry();
			/* Compare taskId. */
			if ((taskId != null) && (!taskEntry.getTaskId().equals(taskId))) {
				continue;
			}
			/* Compare contextId. */
	...
	/**
	 * Remove one <code>TaskNode</code> specified by <code>taskEntry</code> from
	 * system.
	 * 
	 * @param taskEntry
