...
public class TaskManagerRunner {
	...
	private static final String XSD_ROOT;
	
	static {																						// TODO: Remove magic constants.
		final String beenHome = System.getenv("BEEN_HOME");
		if (null == beenHome) {
			XSD_ROOT = "../data/taskmanager";														// Perhaps in full/bin?
		} else {
			XSD_ROOT = beenHome + "/data/taskmanager";												// OK, we can guess the path.
		}
	}
		...
		try {
			...
			if (args.length - argNo > 0) {
				...
				try {
					...
					System.err.println(
						"Running of tasks specified by their task descriptors failed: " + e.getMessage()
					);
				}
			} else {
