...
public class ServicesUtils {
	 ...
	 */
	public static SWRepositoryInfo getSWRepositoryInfo() {
		Object swRepObject = getServicesMap().get(Names.SWREPOSITORY_SERVICES_MAP_KEY);
		try {
			return (SWRepositoryInfo) swRepObject;
		} catch (ClassCastException e) {
			e.printStackTrace();
			throw new RuntimeException(String.format("Object in servicesMap under the key '%s' is instance of type '%' but it should be '%s'",
					Names.SWREPOSITORY_SERVICES_MAP_KEY, swRepObject.getClass().getName(), SWRepositoryInfo.class.getName()), e);
		}
	}
	
	public static void storeSWRepositoryInfo(SWRepositoryInfo swRepositoryInfo) {
		// FIXME Radek Macha:
		// getServicesMap().put(Names.SWREPOSITORY_SERVICES_MAP_KEY, swRepositoryInfo)
	}
	
	public static void removeSWRepositoryInfo() {
		// FIXME Radek Macha:
		// something like
	 ...
	 */
	private static IMap<String, Object> getServicesMap() {
