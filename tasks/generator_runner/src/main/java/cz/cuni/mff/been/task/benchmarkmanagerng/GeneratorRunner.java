...
public class GeneratorRunner extends Job {
	...
	public GeneratorRunner() throws TaskInitializationException {
		try {
			manager = CurrentTaskSingleton.getTaskHandle().getPluggableModuleManager();
	...
	protected void checkRequiredProperties() throws TaskException {
		...
		if( analysis == null ){
			...
			for (String str : ((Analysis)analysis).getGenerator().
					validate(CurrentTaskSingleton.getTaskHandle().getPluggableModuleManager())
			){
				errors.append(str);
				errors.append(";");
			}
	...
	protected void run() throws TaskException {
		...
		try {
			GeneratorInterface generator = (GeneratorInterface)
				analysis.getGenerator().getPluggableModule(manager);
			generator.configure(analysis);
			Collection<TaskDescriptor> tasks = generator.generate();

			if( tasks != null && tasks.size() > 0){
				logInfo("Sending "+tasks.size()+" tasks to the Task Manager");
				getTasksPort().runTasks(tasks.toArray(new TaskDescriptor[tasks.size()]));

			} else {
				logWarning("Generator didn't produce any tasks to run");
				getBMCallbackInterface().reportAnalysisFinish(getTaskDescriptor().getContextId());
			}
		} catch( BenchmarkManagerException e ){
			logError("Can't run generator because error occured: "+e.getMessage());
			exitError();
		} catch (RemoteException e) {
			logError("Can't run generated tasks because error occured: "+e.getMessage());
			exitError();
		...
		try{
			...
			bmng.reportGeneratorSuccess(analysis.getName(), getTaskDescriptor().getContextId(), getTaskDescriptor().getTaskId());
		} catch (RemoteException e) {
			logError("Can't increase the succesful run count because error occured: "+e.getMessage());
		} catch (BenchmarkManagerException e) {
			logError("Can't increase the succesful run count because error occured: "+e.getMessage());
		}
		exitSuccess();
	 ...
	 */
	private BenchmarkManagerCallbackInterface getBMCallbackInterface() throws RemoteException {
		return (BenchmarkManagerCallbackInterface)getTasksPort()
				.serviceFind(BenchmarkManagerService.SERVICE_NAME,
	...
	private TaskDescriptor getMonitorTask() {
		...
		treeAdress = treeAdress.substring(0, treeAdress.lastIndexOf('/'))+"/monitor";
		TaskDescriptor descriptor = TaskDescriptorHelper.createTask("monitor-"+context, "context-monitor", context, null, treeAdress);
		descriptor.getHostRuntimes().setAsTask(getTaskDescriptor().getTaskId());  // TODO - should be restricted to host where Taskmanager is runnig
		// Andrej: No, it shouldn't. What if that host doesn't have any Host Runtime? Such a situation is perfectly legal.
		return descriptor;
	}
