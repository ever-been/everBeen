...
public class HostsModule extends Module implements EventListener {
	 ...
	 * implementing <code>HostInfoInterface</code>) in the host list, so it can
	 * be sorted alphabetically by their hostnames.
	 * 
	 * @author David Majda
	 */
	private static class HostListComparator implements
			Comparator<HostInfoInterface> {
		/**
		 ...
		 * 
		 * @param o1
		 *            information about first host
		 * @param o2
		 *            information about second host
	 ...
	 * @author David Majda
	 */
	private static class HostListInGroupComparator implements
			Comparator<HostInfoInterface> {
		/** Hosts in edited group. */
		private final List<String> groupHosts;
		...
		/**
		 * Compares information about two hosts (objects implementing
		 * <code>HostInfoInterface</code>). Primary comparison key is incidence
		 * to given group (hosts belonging to the group go first), secondary key
		 * is the hostname (comparison is alphabetical and case-insensitive).
		 * 
		 * @param o1
		 *            information about first host
		 * @param o2
	 ...
	 * <code>HostGroup</code> objects) in the group list, so it can be sorted
	 * alphabetically by their names (with the exception that "Universe" group
	 * is allways first).
	 * 
	 * @author David Majda
	 */
	private static class GroupListComparator implements Comparator<HostGroup> {
		/**
		 * Compares information about two groups (<code>HostGroup</code>
		 * objects). Comparison key is the group name (comparison is
		 * alphabetical and case-insensitive). There is one exception:
		 * "Universe" group is allways first.
		 * 
		 * @param o1
		 *            information about first group
		 * @param o2
		 *            information about second group
		 * @return a negative integer, zero, or a positive integer as the first
		 *         argument is less than, equal to, or greater than the second
		 */
		@Override
	 ...
	 * @author David Majda
	 */
	private static class AliasListComparator implements
			Comparator<SoftwareAliasDefinition> {
		/**
		 ...
		 * 
		 * @param o1
		 *            information about first alias definition
		 * @param o2
		 *            information about second alias definition
		 * @return a negative integer, zero, or a positive integer as the first
		 *         argument is less than, equal to, or greater than the second
		 */
		@Override
		public int compare(
				SoftwareAliasDefinition o1,
				SoftwareAliasDefinition o2) {
			return o1.getAliasName().compareToIgnoreCase(o2.getAliasName());
		}
	}

	/**
	 * Allocates a new <code>HostsModule</code> object. Construcor is private so
	...
	private void closeOperationHandle(
			HttpServletRequest request,
			String handleParamName) throws MissingParamException,
			InvalidParamValueException, RemoteException,
		...
		try {
			hostManager.get().removeOperationStatus(
					OperationHandle.valueOf(request
							.getParameter(handleParamName)));
		} catch (IllegalArgumentException e) {
			throw new InvalidParamValueException("Parameter \""
					+ handleParamName + "\" has invalid value.");
		}
	}
	 ...
	 *             if the Host Manager can't be initialized
	 */
	public void operationStatus(
			HttpServletRequest request,
			HttpServletResponse response) throws MissingParamException,
			InvalidParamValueException, ServletException, IOException,
			ComponentInitializationException {
	...
	public void hostList(
		...
		for (String hostName : hostManager.get().getHostNames()) {
			try {
				 ...
				 * changes the host database under our hands. The host simply
	 ...
	 *             if the Host Manager can't be initialized
	 */
	public void hostJavascriptList(
			HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException,
		...
		try {
			for (String hostName : hostManager.get().getHostNames()) {
				try {
					...
					/*
					 * We ignore this exception, which could happen only if
					 * someone changes the host database under our hands. The
					 * host simply won't be included in the list.
					 */
				}
			}
			 ...
			 * We don't want to report anything, just send the client empty list
			 * of hosts.
			 */
		}
	 ...
	 *             if the Host Manager can't be initialized
	 */
	public void hostJavascriptListRsl(
			HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException,
			MissingParamException, InvalidParamValueException,
			ComponentInitializationException {
		...
		String validationResult = RSLValidator.validate(request
				.getParameter("rsl"));
		if (validationResult == null) { /*
										 * RSL is lexically and syntactically
										 * correct.
										 */
			try {
				hosts = hostManager.get().queryHosts(
						new RestrictionInterface[] { new RSLRestriction(request
								.getParameter("rsl")) });
			} catch (ValueNotFoundException e) {
	 ...
	 */
	public void
			hostAdd(HttpServletRequest request, HttpServletResponse response)
					throws MissingParamException, ServletException,
		...
		if (params.exists("add")) {
			...
			params.checkCondition(
					params.notEmpty("hostname"),
					"Enter non-empty hostname.");
			try {
				params.checkCondition(
						!hostManager.get().isHostInDatabase(
								request.getParameter("hostname")),
						"Entered hostname is already in the database.");
			} catch (UnknownHostException e) {
				params.checkCondition(
						false,
						"Unknown hostname: " + request.getParameter("hostname")
								+ ".");
			}
	...
	private Map<String, String> buildUserProperties(
			NameValuePair[] userProperties) {
	...
	public void hostDetails(
		...
		if (params.exists("action")) {
			...
			if (action.equals("refreshed")) {
				closeOperationHandle(request, "handle");
				infoMessages
						.addTextMessage("Host configuration refreshed successfully.");
			} else if (action.equals("user-property-added")) {
				infoMessages
						.addTextMessage("User property added successfully.");
				activeSheet = "user-properties-sheet";
			} else if (action.equals("user-properties-edited")) {
				infoMessages
						.addTextMessage("User properties edited successfully.");
				activeSheet = "user-properties-sheet";
			} else if (action.equals("user-property-deleted")) {
				infoMessages
						.addTextMessage("User property deleted successfully.");
				activeSheet = "user-properties-sheet";
			} else {
				throw new InvalidParamValueException(
						"Parameter \"action\" has invalid value.");
		...
		Date[] dates = null;
		try {
			dates = hostManager.get().getHostHistoryDates(
					request.getParameter("hostname"));
		} catch (ValueNotFoundException e) {
			throw new InvalidParamValueException(
					"Parameter \"hostname\" has invalid value.");
		}
		...
		try {
			...
			errorMessages
					.addTextMessage("Error retrieving information about host: "
							+ e.getMessage() + ".");
		...
		if (params.exists("add")) {
			...
			params.ensureCondition(
					"new-type",
					params.isInSet("new-type", types));

			params.checkCondition(
					params.notEmpty("new-name"),
					"Enter non-empty property name.");
			if (params.notEmpty("new-name")) {
				boolean isValidTypeName = PropertyTreeFactory
						.isValidTypeName(request.getParameter("new-name"));
				params.checkCondition(
						isValidTypeName,
						"Enter valid property name.");
				if (isValidTypeName) {
					params.checkCondition(
							!hostInfo.hasUserProperty(request
									.getParameter("new-name")),
							"Property \"" + request.getParameter("new-name")
									+ "\" already exists.");
				}
			}
			...
			Map<String, ValueCommonInterface> typesToValues = new HashMap<String, ValueCommonInterface>();
			...
			UserPropertiesHandler handler = new UserPropertiesHandler(
					request,
					"new-value",
					false);
			...
			if (errorMessages.isEmpty()) {
				...
				for (NameValuePair property : newProperties) {
					hostInfo.addUserProperty(
							request.getParameter("new-name"),
							property.getValue());
				}
				try {
					...
					errorMessages
							.addTextMessage("Error adding host properties: "
									+ e.getMessage() + ".");
				}
				...
				userProperties = buildUserProperties(hostInfo
						.getUserProperties());
				newUserPropertyName = request.getParameter("new-name");
				newUserPropertyType = request.getParameter("new-type");
				newUserPropertyValue = request
						.getParameter(request.getParameter("new-type").equals(
								VALUE_TYPE_BOOLEAN) ? "new-value-boolean"
								: "new-value-text");
				activeSheet = "user-properties-sheet";
			}
			...
			if (errorMessages.isEmpty()) {
				...
				try {
					...
					errorMessages
							.addTextMessage("Error editing host properties: "
									+ e.getMessage() + ".");
				}
				...
				for (NameValuePair property : properties) {
					...
					userProperties.put(
							property.getName(),
							paramValue != null ? paramValue : property
									.getValue().toString());
				}
				newUserPropertyName = "";
			...
			try {
				...
				throw new InvalidParamValueException(
						"Parameter \"delete\" has invalid value.");
			} catch (InvalidArgumentException e) {
				throw new InvalidParamValueException(
						"Parameter \"delete\" has invalid value.");
			} catch (HostDatabaseException e) {
		...
		try {
			loadData = loadServer.get().getStatsProvider(
					request.getParameter("hostname"));
			try {
				loadData.refresh();
			...
			errorMessages.addTextMessage("Error retrieving load data: "
					+ e.getMessage());
		} catch (ValueNotFoundException e) {
			throw new InvalidParamValueException(
					"Parameter \"hostname\" has invalid value.");
		...
		TaskEntry[] tasks = taskManager.get().getTasksOnHost(
				request.getParameter("hostname"));
		...
		try {
			logRecords = LogUtils.getLogRecordsForTasks(
					taskManager.get(),
					taskManager.get().getTasksOnHost(
							request.getParameter("hostname")));
	...
	public void hostRefresh(
		...
		try {
			handle = hostManager.get().refreshHost(
					request.getParameter("hostname"));
		} catch (ValueNotFoundException e) {
			throw new InvalidParamValueException(
					"Parameter \"hostname\" has invalid value.");
		} catch (UnknownHostException e) {
			throw new InvalidParamValueException("Unknown hostname: "
					+ request.getParameter("hostname") + ".");
		}
	...
	public void groupList(
		...
		for (String groupName : hostManager.get().getGroupNames()) {
			try {
				...
				/**
				 * We ignore this exception, which could happen only if someone
				 * changes the host database under our hands. The group simply
				 * won't be included in the list.
				 */
			}
		}
	...
	public void groupAddOrEdit(HttpServletRequest request, boolean editing)
			throws MissingParamException, InvalidParamValueException,
			ServletException, IOException, ComponentInitializationException {
		HashMap<String, Object> data = new HashMap<String, Object>();

		if ((editing && params.exists("edit"))
				|| (!editing && params.exists("add"))) {
			HostGroup group = null;
			if (editing) {
				params.ensureExists("group");
				try {
					group = hostManager.get().getGroup(
							request.getParameter("group"));
				} catch (ValueNotFoundException e) {
					throw new InvalidParamValueException(
							"Parameter \"group\" has invalid value.");
				}
				...
				params.ensureExists("name");
				/*
				 * If the group name is empty or equal to "Universe", create
				 * temporary group with random name, because HostGroup
				 * constructor doesn't allow empty group name and will mess up
				 * when creating group with name "All hosts".
				 * 
				 * This is to simplify the code bellow, where we can assume that
				 * the group object is allways created.
				 * 
				 * Incorrect group name doesn't harm anything, because if the
				 * "name" parameter is empty or equal to "Universe", group won't
				 * be added to the Host Manager.
				 */
				if (params.isEmpty("name")
						|| request.getParameter("name").equals(
								HostGroup.DEFAULT_GROUP_NAME)) {
					try {
			...
			if (!group.isDefaultGroup()) {
				for (int i = 0; i < hostCount; i++) {
					String paramName = params.makeIndexed(
							"group-hosts",
							hostNames[i]);
					if (params.exists(paramName)) {
						params.ensureCondition(
								paramName,
								params.isCheckboxBool(paramName));
					}
				}
			}
			...
			if (errorMessages.isEmpty()) {
				...
				data.put("name", group.isDefaultGroup() ? group.getName()
						: request.getParameter("name"));
				...
				for (String hostName : hostNames) {
					try {
						...
						/*
						 * We ignore this exception, which could happen only if
						 * someone changes the host database under our hands.
						 * The host simply won't be included in the list.
						 */
					}
				}
				...
				Collections.sort(hosts, new HostListInGroupComparator(
						groupHosts));
	...
	public void aliasList(
		...
		for (int i = 0; i < hostManager.get().getAliasDefinitionCount(); i++) {
			try {
				...
				/*
				 * We ignore this exception, which could happen only if someone
				 * changes the software alias definition database under our
				 * hands. The software alias definition simply won't be included
				 * in the list.
				 */
			}
		}
	...
	private SoftwareAliasDefinition findSoftwareAliasDefinitionByName(
			String aliasName) throws RemoteException,
			ComponentInitializationException {
		...
		try {
			 ...
			 * The software alias definition simply won't be included in the
			 * list.
			 */
		}
		return alias;
	...
	private void deleteSoftwareAliasDefinitionByName(String aliasName)
			...
			HostDatabaseException {
		try {
			hostManager.get().removeAliasDefinitionByName(aliasName);
		} catch (ValueNotFoundException e) {
			/*
			 * This function has a different behaviour than the find... version.
			 * It used to be a terrible non-atomic read-modify-write stuff that
			 * distinguished between two situations (alias not found initially
			 * and alias removed concurrently). However, this operation should
			 * be atomic and both of the two situations must result in the same
			 * behaviour. Strictly speaking, there is (from the theoretical
			 * point of view) absolutely no way to tell one of them from the
			 * other.
			 */
	...
	private void checkRSLParam(
		...
		if (validationResult != null) {
			errorMessages.addHTMLMessage(paramDescription + ": "
					+ validationResult);
		}
	}

	public void aliasAddOrEdit(HttpServletRequest request, boolean editing)
			throws MissingParamException, InvalidParamValueException,
			ServletException, IOException, ComponentInitializationException {
		HashMap<String, Object> data = new HashMap<String, Object>();
		...
		if ((editing && params.exists("edit"))
			...
			params.checkCondition(
					params.notEmpty("alias-name"),
					"Enter non-empty alias name.");
			if (editing) {
				params.checkCondition(
						request.getParameter("alias-name").equals(
								request.getParameter("alias"))
								|| findSoftwareAliasDefinitionByName(request
										.getParameter("alias-name")) == null,
			...
			try {
				osRestriction = !request.getParameter("os-restriction").equals(
						"") ? new RSLRestriction(
						request.getParameter("os-restriction")) : null;
				appRestriction = new RSLRestriction(
						request.getParameter("app-restriction"));
			} catch (IllegalArgumentException e) {
			...
			if (errorMessages.isEmpty()) {
				try {
					if (editing) {
						try {
							deleteSoftwareAliasDefinitionByName(request
									.getParameter("alias"));
						} catch (IllegalArgumentException e) {
							throw new InvalidParamValueException(
									"Parameter \"alias\" has invalid value.");
						}
					}
			...
			if (errorMessages.isEmpty()) {
				...
				data.put(
						"osRestriction",
						request.getParameter("os-restriction"));
				data.put(
						"appRestriction",
						request.getParameter("app-restriction"));
			}
			...
			if (editing) {
				params.ensureExists("alias");
				SoftwareAliasDefinition alias = findSoftwareAliasDefinitionByName(request
						.getParameter("alias"));
				...
				if (null != alias) {
					...
					RSLRestriction osRestriction = (RSLRestriction) alias
							.getOsRestriction();
					data.put(
							"osRestriction",
							osRestriction != null ? osRestriction
									.getRSLString() : "");
					RSLRestriction appRestriction = (RSLRestriction) alias
							.getAppRestriction();
		...
		page.setTitle(editing ? "Edit software alias definition: "
				+ data.get("aliasName") : "Add software alias definition");
	...
	public void aliasDelete(
		...
		try {
			...
			errorMessages.addTextMessage("Error deleting alias: "
					+ e.getMessage() + ".");
		}
	 ...
	 */
	public void
			rslHelp(HttpServletRequest request, HttpServletResponse response)
					throws MissingParamException, InvalidParamValueException,
		...
		page.setTitle("RSL help ("
				+ (request.getParameter("type").equals(RSL_HELP_TYPE_OS) ? "operating system"
