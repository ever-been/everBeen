...
public class TaskContexts {
     ...
     *
     * @param descriptor descriptor of a context
     ...
     *
     * @param taskContextEntry entry to clean up
     */
    ...
    public void kill(String taskContextId) {
        ...
        if (state == TaskContextState.FAILED || state == TaskContextState.FINISHED) {
            throw new IllegalStateException(String.format("Trying to kill task context %s, but it's in state %s.", taskContextId, state));
        }

        List<String> containedTasks = taskContextEntry.getContainedTask();

        getTaskContextsMap().lock(taskContextId);
        taskContextEntry.setContextState(TaskContextState.FAILED);
        for (String taskId : containedTasks) {
            TaskEntry taskEntry = getTask(taskId);
            if (taskEntry != null) {
                KillTaskMessage killMessage = new KillTaskMessage(taskEntry.getRuntimeId(), "killed by user", taskId);
                clusterContext.getTopics().publishInGlobalTopic(killMessage);
            }
        }
        getTaskContextsMap().unlock(taskContextId);
    }

    private TaskEntry getTask(String taskId) {
