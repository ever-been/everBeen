...
/**
 * This class acts as an adapter to the native libraries that are provided with
 * the Detector Task. It will attempt to initialize correct library and query
 * data about the host. If an error occured while initializing native library or
 * if no native library has been found for current platform, only primitive Java
 * implementation of the detector will be used.
 * 
...
public class NativeDetector {
	 ...
	 * 
	 * @return <code>true</code> if successful, <code>false</code> otherwise. It
	 *         should always return <code>true</code> since native detectors are
	 *         written so that in case of error they can collect data not
	 *         affected by given error.
	 */
	 ...
	 * 
	 * @return Detector data in string. This is XML file data converted to
	 *         string.
	 */
	private native String nativeGetData();

	/**
	 * Get messages produced by the native detector. These messages may contain
	 * info about errors in the native library or successful operations.
	 * 
	 * @return String containing all messages. Each message is formated on one
	 *         line.
	 */
	...
	/**
	 * Flag which specifies whether detection will be done using native
	 * libraries.
	 */
	...
	/**
	 * Constructor. This will attempt to load native library based on the
	 * Operating System on which it is run. If corresponding native library is
	 * not found or initialization fails, using_native flag will be set to
	 * false.
	 */
	public NativeDetector() {
		...
		try {
			...
			usingNative = false;
			task.logError("Unable to load native detector library, message: \""
					+ e.getMessage() + "\".");
		} catch (Error e) {
			usingNative = false;
			task.logError("Unable to load native detector library, message: \""
	...
	/**
	 * Do all the work - call native methods or detect everything with Java and
	 * compose resulting data.
	 * 
	...
	public boolean execute() {
		...
		if (usingNative) {
			...
			// ...and finally get any error reports we may have generated in
			// native code.
			messages = nativeGetMessages();
			...
			// No native lib for current os, we will use Java to get very
			// limited data.
			...
			nativeData = "<?xml version=\"1.0\" encoding=\"UTF-16\"?>\n"
					...
					+ makeXMLElement(
							"hostName",
							MiscUtils.getCanonicalLocalhostName(),
							1)
					+ makeXMLElement(
							"lastCheckDate",
					...
					+ makeXMLElement(
							"detector",
							HostInfoInterface.Detectors.GENERIC.toString(),
							1)
					+ "\t<operatingSystem>\n"
					+ "\t\t<basicInfo>\n"
					...
					+ makeXMLElement("freeSpace", "-1", 2) + "\t</beenDisk>\n";
		...
		if (usingNative) {
			...
			data = nativeData.substring(0, pos) + javaInfo
					+ nativeData.substring(pos);

			/*
			 * This is not very nice, since we replace data from native detector
			 * with the data detected by Java, but it is needed for consistency.
			 */
			int hnPos1 = data.indexOf("<hostName>");
			int hnPos2 = data.indexOf("</hostName>");

			try {
				data = data.substring(0, hnPos1) + "<hostName>"
	...
	/**
	 * Create string representing XML element with given name and value at the
	 * given column (columns are tabs).
	 * 
	 * @param name
	 ...
	 * @param pos
	 *            Number of tabs before the opening angle bracket of the
	 *            element.
