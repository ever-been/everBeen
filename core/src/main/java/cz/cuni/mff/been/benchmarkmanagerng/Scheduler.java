...
public class Scheduler extends Thread {
	...
	private final BenchmarkManagerInterface benchmarkManager;
	private final Task task;

	private final HashMap<String, Integer> blockedAnalyses = new HashMap<String, Integer>();
	...
	public void run() {
		...
		while (true) {
			...
			try {
				...
				dumpException(
						"Couldn't load analyses because error occured: ",
						e);
				continue; // i.e. go to sleep
			} catch (RemoteException e) {
				assert false : "RemoteException occured in local call: "
						+ e.toString();
				continue;
			}
			Integer blockCount;
			for (Analysis analysis : analyses) {
				if (analysis.shouldBeScheduled()) {
					try {
						...
						if (blockCount == null) {
							blockCount = 0;
							logWarning("Couldn't run analysis '"
									+ analysis.getName()
									+ "' because error occured: "
									+ e.getMessage());
						}
						blockCount++;
	...
	private void dumpException(String message, Throwable e) {
		...
		System.err.println(message + " because error occured: "
				+ e.getMessage());
		e.printStackTrace();
		System.err.println("----------------------------------------");
	}
	...
	/**
	 * Logs a warning message. It uses task logging facility to write the
	 * message if possible, otherwise it prints it to the standard output.
	 * 
	 * @param message
	 *            message text
	 */
	...
	/**
	 * Logs an error message. It uses task logging facility to write the message
	 * if possible, otherwise it prints it to the error output.
	 * 
	 * @param message
	 *            message text
	 */
