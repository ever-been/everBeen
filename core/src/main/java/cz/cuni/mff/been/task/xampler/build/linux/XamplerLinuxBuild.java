...
public class XamplerLinuxBuild extends Job {
	...
	protected void run() throws TaskException {
		...
		// File omniorbArchive = new
		// File("/home/johny/Desktop/been_data/omniorb-binary.tar.gz");
		File xamplerSrcArchive = new File(
				getTempDirectory(),
				"xampler-src.tar.gz");
		// File xamplerSrcArchive = new
		// File("/home/johny/Desktop/been_data/xampler-source.tar.gz");
		File xamplerBinArchive = new File(
				getWorkingDirectory(),
				"xampler-bin.tar.gz");
		...
		try {
			Untar.untar(
					omniorbArchive.getAbsolutePath(),
					omniorbRoot.getAbsolutePath(),
					"gzip");
		...
		try {
			Untar.untar(
					xamplerSrcArchive.getAbsolutePath(),
					xamplerSrcRoot.getAbsolutePath(),
					"gzip");
		} catch (AntTaskException e) {
		...
		if (!simulate) {
			...
			try {
				...
				String[] script = new String[] {
						...
						"	cd ../..",
						"	echo \"Suite $SUITE built successfully.\" >&2",
						"done",
			...
			try {
				Move.filesetMove(
						xamplerSrcRoot.getAbsolutePath(),
						"**/Server",
						xamplerBinRoot.getAbsolutePath());
				Move.filesetMove(
						xamplerSrcRoot.getAbsolutePath(),
						"**/Client",
						xamplerBinRoot.getAbsolutePath());
				...
				throw new TaskException(
						"Collecting Xampler binaries failed.",
						e);
			}
		...
		if (!buildFailed) {
			try {
				Tar.tar(
						xamplerBinRoot.getAbsolutePath(),
						xamplerBinArchive.getAbsolutePath(),
						"gzip");
			} catch (AntTaskException e) {
	...
	protected void checkRequiredProperties() throws TaskException {
		checkRequiredProperties(new String[] { RR_SRC_ANALYSIS_NAME,
				...
				RR_OMNIORB_ANALYSIS_NAME, RR_OMNIORB_DATASET_NAME,
				XAMPLER_REVISION, OMNIORB_TIMESTAMP, XAMPLER_SUITE_SUBDIR,
				BUILD_NUMBER });
		...
		try {
			...
			throw new TaskException("Error parsing \"" + BUILD_NUMBER
					+ "\" property");
		}
		...
		try {
			...
			throw new TaskException("Error parsing \"" + XAMPLER_REVISION
					+ "\" property");
		}
