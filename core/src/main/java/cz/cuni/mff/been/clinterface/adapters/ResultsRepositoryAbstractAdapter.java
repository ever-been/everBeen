...
/**
 * Abstract ancestor of all Results Repository adapters, with some common utility methods.
 * 
...
abstract class ResultsRepositoryAbstractAdapter {
	...
	/**
	 * Initializes the Results Repository reference using the supplied Task Manager reference.
	 * 
	 * @param taskManagerReference A reference to query the Task Manager.
	 */
	protected ResultsRepositoryAbstractAdapter( TaskManagerReference taskManagerReference ) {
		this.rrManagerReference = new ServiceReference< RRManagerInterface >(
			taskManagerReference,
			ResultsRepositoryService.SERVICE_NAME,													// TODO: One name, 2 ifaces???
	...
	throws ResultsRepositoryException, RemoteException, ComponentInitializationException {
		...
		for ( String analysis : rrmi.getAnalyses() ) {
			result.add( Pair.pair( analysis, rrmi.getDatasets( analysis ) ) );						// getDatasets() doesn't throw!
		}
		return result;
	}
	 ...
	 * 
	 * @param pattern The regular expression analysis names must match.
	 * @return  A filtered list of analyses and their datasets.
	 * @throws ResultsRepositoryException When something bad happens in the Results Repository.
	 * @throws RemoteException When it rains.
	 * @throws ComponentInitializationException When the Service Reference reports a failure.
	 */
	public Iterable< Pair< String, ? extends Iterable< String > > > getAnalyses( Pattern pattern )
	throws ResultsRepositoryException, RemoteException, ComponentInitializationException {
		RRManagerInterface rrmi;
		ArrayDeque< Pair< String, ? extends Iterable< String > > > result;
	 ...
	 * @throws ComponentInitializationException When the Service Reference reports a failure.
	 */
	public Iterable< Pair< String, DatasetDescriptor > > getDatasets( String analysis )
	throws RemoteException, ResultsRepositoryException, ComponentInitializationException {
		RRManagerInterface rrmi;
		...
		for ( String dataset : rrmi.getDatasets( analysis ) ) {
			try {
				result.add( Pair.pair( dataset, rrmi.getDatasetDescriptor( analysis, dataset ) ) );	// This may throw! Ugly API!!!
	 ...
	 * 
	 * @param analysis Name of the analysis.
	 * @param pattern The regular expression dataset names must match.
	...
	public Iterable< Pair< String, DatasetDescriptor > > getDatasets(
		...
		for ( String dataset : rrmi.getDatasets( analysis ) ) {
			if ( pattern.matcher( dataset ).matches() ) {
				try {
					result.add(
						Pair.pair( dataset, rrmi.getDatasetDescriptor( analysis, dataset ) )		// This may throw! Ugly API!!!
					);
				} catch ( ResultsRepositoryException exception ) {
					Task.getTaskHandle().logWarning(
						"Dataset (" + analysis + ", " + dataset + ") is gone."
	 ...
	 * 
	 * @param analysis Name of the analysis to which the dataset belongs.
	 ...
	 * @param analysis Name of the analysis.
	 * @param dataset Name of the dataset.
	 * @param pattern The regular experssion evaluator names must match.
	 * @return A filtered list of triggers bound to the analysis and dataset.
	 * @throws ResultsRepositoryException when something bad happens in the Results Repository.
	 * @throws RemoteException When it rains.
	 * @throws ComponentInitializationException When the Service Reference reports a failure.
	 */
	public Iterable< RRTrigger > getTriggers( String analysis, String dataset, Pattern pattern )
	throws ResultsRepositoryException, RemoteException, ComponentInitializationException {
		ArrayDeque< RRTrigger > result;
		
		result = new ArrayDeque< RRTrigger >();
		for ( RRTrigger trigger : rrManagerReference.get().getTriggers( analysis, dataset ) ) {
	 ...
	 */
	public void deleteTrigger( UUID triggerID ) throws
	 ...
	 * @param analysis Name of the analysis.
	 * @param dataset Name of the dataset.
	 * @param evaluator Name of the evaluator used by the trigger to delete.
	 * @throws ResultsRepositoryException When something bad happens in the Results Repository.
	 * @throws RemoteException When it rains.
	 * @throws ComponentInitializationException When the Service Reference reports a failure.
	 */
	public void deleteTriggers( String analysis, String dataset, String evaluator )
	throws ResultsRepositoryException, RemoteException, ComponentInitializationException {
		rrManagerReference.get().deleteTriggers( analysis, dataset, evaluator );
	}
	
	/**
	 * Downloads a file from the Results Repository and writes its contents into a stream. 
	 * 
	 * @param uuid Unique identifier of the file to download.
	 * @param output The stream to write the file.
	 * @throws IOException When the file upload fails.
	 * @throws ComponentInitializationException When the Service Reference reports a failure.
	 * @throws RemoteException When it rains.
	 */
	public void downloadFile( UUID uuid, OutputStream output )
	throws RemoteException, ComponentInitializationException, IOException {
		rrDataReference.get().getFileStoreClient().downloadFile( uuid, output );
	}
	
	/**
	 * Uploads a file into the Results Repository, reading its contents from a stream.
	 * 
	 * @param input The stream to read the file.
	 * @return Unique identifier assigned to the new file.
	 * @throws RemoteException When it rains.
	 * @throws ComponentInitializationException When the Service Reference reports a failure.
	 * @throws IOException When the file upload fails.
	 */
	public UUID uploadFile( InputStream input )
	throws RemoteException, ComponentInitializationException, IOException {
