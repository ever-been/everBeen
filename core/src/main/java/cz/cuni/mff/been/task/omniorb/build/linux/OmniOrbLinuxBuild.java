...
public class OmniOrbLinuxBuild extends Job {
	...
	protected void run() throws TaskException {
		...
		if (!simulate) {
			...
			if (patch) {
				File omniorbPatch = new File(
						getTaskDirectory(),
						OMNIORB_PATCH_FILENAME);
			...
			if (!buildFailed)
				logInfo("omniORB sucessfully built.");
		} else {
			binaryRoot.mkdirs();
		}
		...
		if (!buildFailed) {
			try {
				Tar.tar(
						binaryRoot.getAbsolutePath(),
						binArchive.getAbsolutePath(),
						"gzip");
			} catch (AntTaskException e) {
	...
	protected void checkRequiredProperties() throws TaskException {
		checkRequiredProperties(new String[] { TIMESTAMP, BUILD_NUMBER,
				RR_SRC_ANALYSIS_NAME, RR_SRC_DATASET_NAME,
				RR_BIN_ANALYSIS_NAME, RR_BIN_DATASET_NAME, });
