...
/**
 * <p>
 * Performs a checkout from a Subversion repository. If no revision is
 * specified, then it will checkout the HEAD.
 * </p>
 * 
 ...
 * - if this task property is set, then the revision number in its value will be
 * checked out from the SVN repository<br>
 * <li>{@value #TIMESTAMP}: <br>
 * - if this task property is set, then the revision from the time specified in
 * its value will be checked out from the SVN repository. The time must be
 * specified in milliseconds from epoch. <br>
 * </ul>
 * 
...
public class SVNCheckout extends Job {
	...
	private final boolean revisionHead = false;
	...
	protected void checkRequiredProperties() throws TaskException {
		...
		if (tstamp != null) {
			try {
				...
				throw new TaskException("Error parsing \"" + TIMESTAMP
						+ "\" property");
			}
		}
		...
		if (rev != null) {
			try {
				...
				throw new TaskException("Error parsing \"" + REVISION
						+ "\" property");
			}
		}

		if ((timestamp != null) && (revision != null)) {
			throw new TaskException("Both \"" + REVISION + "\" and \""
					+ TIMESTAMP + "\" properties are specified at a time.");
		}

		if ((timestamp == null) && (revision == null)) {
			throw new TaskException("None of \"" + REVISION + "\" and \""
					+ TIMESTAMP + "\" properties specified.");
		}
	...
	protected void run() throws TaskException {
		...
		if (!simulate) {
			try {
				client.doCheckout(SVNURL.parseURIEncoded(url), new File(
						getTempDirectory()), rev, rev, true);
		...
		try {
			...
			FileAgent agent = fileAgentModule.createRRFileAgent(
					analysisName,
					datasetName,
					FILE_ID_FIELD_NAME);
			...
			if (timestamp != null) {
				// TODO A temporary hack to work around timestamp definition
				// mismatch.
				// While the checkout task expects the timestamp to be a long
				// integer,
				// the surrounding generator structure defines the timestamp as
				// a
	...
	private void loadPluggableModules() throws TaskException {
		...
		try {
			fileAgentModule = CurrentTaskSingleton.getTaskHandle()
					.getPluggableModule(
							FileAgentPluggableModule.class,
							"fileagent",
							"1.0");
