...
/**
 * This is an abstract ancestor of all modules implementing the functionality of command line
 * interface. There is a similar mechanism in webinterface package ({@code Module}), but there
 * are many differences between the two concepts. In the command line interface:
 * <ol>
...
public abstract class CommandLineModule {
	...
	private static final Map< String, Class< ? extends CommandLineModule > >
	classMap;

	/** A mapping of module names to module instance pools. */ 
	private static final Map< String, ConcurrentLinkedQueue< CommandLineModule > >
	instanceMap;

	/** A mapping of module classes to module instance pools. */
	private static final Map<
		Class< ? extends CommandLineModule >,
		ConcurrentLinkedQueue< CommandLineModule >
	>
	reverseMap;
	...
	static {
		...
		/*
		 * You have to add all your module classes here. Unfortunately, module registration cannot
		 * be done from the descendant's static initializer. The JVM will not load its class when
		 * it doesn't seems to be needed, so the initializer would not run at all.
		 */
	...
	/**
	 * This is similar to the mechanism used by the web interface. However, actions can be dynamic
	 * here and it is up to the module to make a run-time decision about action name's correctness.
	 * 
	 * @param action The requested action name.
	 * @param request Command line options obtained from the native client.
	 * @param response A connection to the native client's back channel.
	 * @throws ModuleOutputException When output cannot be written.
	 * @throws ModuleSpecificException When the input is not semantically correct. 
	 */
	 ...
	 * 
	 * @return A list of actions where {@code "\n\t"} is used as prefix and separator.
	 */
	protected abstract String getActionsList();
	
	/**
	 * This should initialize a module and restore any state it may store. Used by the module
	 * instance pool to re-use instances.
	 */
	protected abstract void restoreState();

	/**
	 * Module name getter. Not needed here, but many BEEN components have this feature,
	 * so it might be advisable to implement it, too. 
	 * 
	 * @return The official module name that should be used in mappings and the like.
	 */
	protected abstract String getName();
	
	/**
	 * Disposes the module and stores its instance in its instance pool for future re-use.
	 * Restores its original state so that it can be used again.
	 */
	final void recycle() {
		restoreState();
	...
	/**
	 * Module instance getter. Looks up an instance in the instance pools and creates a new one
	 * if either no instances are available or all instances are busy.
	 * 
	 * @param name Name of the requested module.
	 * @return An instance of the module found by name.
	 * @throws IllegalInputException When an unknown module name is requested.
	 * @throws CommandLineException When module instantiation fails.
	 */
	...
	/**
	 * This registers a module class and maps its name to this class. It should be only invoked
	 * statically on class initialization. It doesn't include any synchronization.
	 * 
	 * @param name Name of the new module for the {@link #forName(String)} lookup.
	 * @param classs A class implementing this module.
	 */
	 ...
	 * 
	 * @param strings Elements of the set.
	 * @return The set.
	 */
	...
	/**
	 * This ads a single instance of a module to the instance pool. This method must be called
	 * by {@link #recycle()} only.
	 * 
	 * @param instance The instance that should be added to the pool.
	 */
	private static void registerInstance( CommandLineModule instance ) {
		...
		if ( ( queue = reverseMap.get( instance.getClass() ) ) == null ) {
			Task.getTaskHandle().logWarning(
				"Unknown module class refused: " + instance.getClass().getName()
			);																						// Weird error, old module etc.
		} else {
	...
	/**
	 * Makes all instances in the current pool inaccessible, so that they can be garbage-collected.
	 * This method could be called after an extremely heavy load, but won't be necessary
	 * in most cases.
	 * 
	 * @param name The module name for which instance pool should be flushed.
	 */
	static void flushPool( String name ) {
		ConcurrentLinkedQueue< CommandLineModule > queue;

		if ( ( queue = instanceMap.get( name ) ) == null ) {										// Module already removed...
			Task.getTaskHandle().logWarning( "Invalid pool flush refused: " + name );
		} else {
			queue.clear();
		}
	}
	...
	/**
	 * Flushes all module instance pools. This method could be called after an extremely heavy
	 * load, but won't be necessary in most cases.
	 */
	static void flushAllPools() {
		for ( ConcurrentLinkedQueue< CommandLineModule > queue : instanceMap.values() ) {
			queue.clear();
		}
	}
}
