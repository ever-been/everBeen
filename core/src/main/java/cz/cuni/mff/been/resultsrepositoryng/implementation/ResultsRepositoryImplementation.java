 ...
 *
 */
public class ResultsRepositoryImplementation extends UnicastRemoteObject implements RRManagerInterface, RRDataInterface {
	 ...
	 * Task handle 
	 */
	private Task task = Task.getTaskHandle();
	
	
	/**
	 * Existing datasets list (each dataset contains also list of associated triggers)
	 */
	...
	/** Triggers that were fired and associated notifyDataProcessed wasn't called yet.
	 */
	private Set<UUID> firedTriggers = new HashSet<UUID> ();
	
	/**
	 * Number of queued events for given trigger.
	 */
	private Map<UUID, Integer> discardedTriggerEvents = new HashMap<UUID, Integer> ();
	
	/** Dynamic entity classes for existing datasets */
	private Map<DatasetName, Class<? extends DynamicEntity> > entityClasses = new HashMap<DatasetName, Class<? extends DynamicEntity> > (); 
	 ...
	 */
	public ResultsRepositoryImplementation(HibernatePluggableModule hibernate, String databaseName, String fileStoreHomeDir) throws ResultsRepositoryException, RemoteException {
		super();		
	...
	/** 
	 * Hibernate session factory for RR dataset metadata (dataset and trigger catalog)
	 */
	 ...
	 */
	private Map<DatasetName, SessionFactory> datasetSessionFactories = new HashMap<DatasetName, SessionFactory> ();
	...
	private void initialize() throws ResultsRepositoryException {
		...
		try {
			...
			evaluatorScheduler = new EvaluatorScheduler() {
				...
				@Override
				public void scheduleEvaluator(TaskDescriptor task,
						UUID triggerId, long lastProcessedSerial)
					...
					try {
						TasksPortInterface tasksPort = ResultsRepositoryImplementation.this.task.getTasksPort();
						TaskManagerInterface taskManager = tasksPort.getTaskManager();
						...
						if( !taskManager.isContextRegistered(context) ){
							...
							taskManager.newContext(context, context, description, context, true);
						}

						tasksPort.runTask(task);
					} catch(RemoteException e) {
						throw new ResultsRepositoryException("Error running evaluator task", e);
			...
			Thread.currentThread().setContextClassLoader(EntityClassFactory.getEntityClassLoader());
			...
			mainSessionFactory = hibernatePluggableModule.createSessionFactory( derbyUrl, annotatedClasses );
		
			session = mainSessionFactory.openSession();
			reloadMetadata(session);
		
			for(RRDataset dataset : datasets) {
				DatasetName qname = new DatasetName(dataset.getAnalysis(), dataset.getName());
			...
			throw new ResultsRepositoryException("Error initializing results repository.", e);
		} finally {
	...
	private long getDatasetMaxSerial(RRDataset dataset) {
		...
		try {
			Class<?> entityClass = entityClasses.get(new DatasetName(dataset.getAnalysis(), dataset.getName()));
			SessionFactory sf = datasetSessionFactories.get(new DatasetName(dataset.getAnalysis(), dataset.getName()));

			session = sf.openSession();

			Query q = session.createQuery("SELECT MAX("+DynamicEntity.SERIAL_FIELD_NAME+ ") FROM " + entityClass.getName());
			List<?> result = q.list();
	...
	/**
	 * Creates hibernate session factory that will be used 
	 * for accessing data in given dataset
	 * @param dataset dataset for which to create hibernate session factory
	...
	private SessionFactory createDatasetSessionFactory(RRDataset dataset) {
		...
		SessionFactory sessionFactory = hibernatePluggableModule.createSessionFactory(getDerbyUrl(), new Class<?>[] { entityClass } );
		return sessionFactory;
	}
	
	/**
	 * Creates dynamic entity instance
	 * @param qname qualified name of dynamic entity
     ...
     */
	@Override
	public synchronized List<DataHandleTuple> loadData(String analysisName, String datasetName, Condition condition, Long fromSerial, Long toSerial)
			throws RemoteException, ResultsRepositoryException {
		return loadData(analysisName, datasetName, condition, fromSerial, toSerial, null);
	}
     ...
     * @param condition condition that returned entries must meet (if null this constraint is not applied)
     * @param fromSerial record's serial number lower bound (if null this constraint is not applied)
     * @param externalSession session to use, or null if session should be retrieved automatically
     * @return all data from dataset which meet given condition.
     * @throws RemoteException
     * @throws ResultsRepositoryException 
     */
	synchronized List<DataHandleTuple> loadData(String analysisName, String datasetName, Condition condition, Long fromSerial, Long toSerial, Session externalSession) 
		throws RemoteException, ResultsRepositoryException {
		...
		if (entityClass == null) {
			throw new ResultsRepositoryException("Dataset \""+qname+"\" does not exist.");
		}
		
		try {
			RRDataset dataset = new RRDataset(analysisName, datasetName, getDatasetDescriptor(analysisName, datasetName));
			
			/* use external session if supplied */
			session = ( externalSession != null ? externalSession : sessionFactory.openSession());
			...
			if (fromSerial != null) {
				query.add( Restrictions.ge(DynamicEntity.SERIAL_FIELD_NAME, fromSerial) );
			}
			if (toSerial != null) {
				query.add( Restrictions.le(DynamicEntity.SERIAL_FIELD_NAME, toSerial) );
			}
			...
			for(Object row : query.list()) {
				...
				DataHandleTuple tuple = dynamicEntity.toDataHandleTuple(dataset.getDatasetDescriptor());			
				result.add(tuple);
			}
			
			return result;
		} catch(Exception e) {
			throw new ResultsRepositoryException("Error occured while loading dataset's data.", e);
		} finally {
	 ...
	 */
	@Override
	public synchronized long saveData(String analysisName, String datasetName, DataHandleTuple data) 
		throws RemoteException, ResultsRepositoryException {
	 ...
	 * @param datasetName name of dataset to save to
	 * @param data data to save
	 * @param externalSession session to use for saving
	 * 
	 * @return serial number assigned to new record
	 ...
	 * @throws ResultsRepositoryException 
	 */
	synchronized long saveData(String analysisName, String datasetName, DataHandleTuple data, Session externalSession)
			throws RemoteException, ResultsRepositoryException {
		...
		try {
			...
			RRDataset dataset = new RRDataset(analysisName, datasetName,
			...
			for (String dataTagName : dataset.getDatasetDescriptor().dataTags()) {
				...
				if (dataHandle == null) throw new ResultsRepositoryException("Data tag \""+dataTagName+"\" is not specified.");
			}
			
			if (dataset.getDatasetDescriptor().idTags().size() > 0) {
				/* if dataset has key, delete record with the same key before saving */
				Criteria criteria = session.createCriteria( entityClasses.get(qname) );

				for (String idTagName : dataset.getDatasetDescriptor().idTags()) {
					DataHandle dataHandle = data.get(idTagName);
					if (dataHandle == null) throw new ResultsRepositoryException("Id tag \""+idTagName+"\" is not specified.");
					Object value = dataHandle.getValue( dataHandle.getType().getJavaType() );				
			...
			throw new ResultsRepositoryException("Error occured while saving data.", e);
		} finally {
			if (externalSession == null) {
				if (session != null) session.close();
			}
	...
	/**
	 * If trigger with the same UUID is not running, fires the given trigger (submits evaluator 
	 * task to taskmanager). 
	 * 
	 * If trigger was called before and corresponding notifyDataProcessed 
	 * haven't been called yet, just saves information that trigger's event
	 * have been discarded (trigger will be fired after notifyDataProcessed 
	 * will be called).
	 * 
	 * 
	 * @param trigger
	...
	private synchronized void fireTrigger(RRTrigger trigger) {
		...
		if (!firedTriggers.contains(triggerId)) {
			if (trigger.getLastProcessedSerial() < serialNumberCounter.getCurrentSerial()) {
				try {
					...
					logInfo("Fired trigger "
							+trigger.getAnalysis()+'/'+trigger.getDataset()+'/'
							+trigger.getEvaluator()+'/'+trigger.getTriggeredTask().getName());

				} catch (ResultsRepositoryException e) {
					logError("Couldn't fire trigger "
							+trigger.getAnalysis()+'/'+trigger.getDataset()+'/'
							+trigger.getEvaluator()+'/'+trigger.getTriggeredTask().getName()
			...
			int discardedEvents = discardedTriggerEvents.get(triggerId).intValue();
			discardedEvents ++;
	...
	private void logError(String message) {
		if( task != null ){ task.logError(message); }
	}
	...
	private void logInfo(String message) {
		if( task != null ){ task.logInfo(message); }
	}

	/**
	 * Schedules evaluator task associated with trigger
	 * (and sets appropriate evaluator's properties).
	 * @param trigger trigger
	 * @throws RemoteException
	 */
	private void scheduleEvaluator(RRTrigger trigger) throws ResultsRepositoryException {
		TaskDescriptor taskDescriptor = trigger.getTriggeredTask();
		
		
		
		
		evaluatorScheduler.scheduleEvaluator(taskDescriptor, trigger.getId(), trigger.getLastProcessedSerial());
	}

	/*
	 * (non-Javadoc)
	 * @see cz.cuni.mff.been.resultsrepositoryng.RRManagerInterface#createDataset(java.lang.String, cz.cuni.mff.been.resultsrepositoryng.data.DatasetDescriptor)
	 */
	@Override
	public synchronized void createDataset(String analysis, String name, DatasetDescriptor descriptor)
		...
		for(RRDataset dataset : datasets) {
			if (qname.equals(  new DatasetName(dataset.getAnalysis(), dataset.getName()))) {
				throw new ResultsRepositoryException("Dataset \"" + qname+ "\" already exists.");
			}
		}
		...
		try {
			...
			datasetSessionFactories.put(qname, createDatasetSessionFactory(newDataset));
		
		} catch(Exception e) {
			if (t != null) t.rollback();
			throw new ResultsRepositoryException("Error adding dataset \""+ name +"\".", e);
		} finally {
	 ...
	 * @param newDataset dataset to check
	 */
	private void checkRRDataset(RRDataset newDataset) throws ResultsRepositoryException {
		if (!isJavaIdentifier(newDataset.getAnalysis())) {
			throw new ResultsRepositoryException("Analysis name cannot be \"" + newDataset.getAnalysis() + "\"");
		}
		if (!isJavaIdentifier(newDataset.getName())) {
			throw new ResultsRepositoryException("Dataset's name cannot be \"" + newDataset.getAnalysis() + "\"");
		}
	}
	...
	/*
	 * (non-Javadoc)
	 * @see cz.cuni.mff.been.resultsrepositoryng.RRManagerInterface#createTrigger(cz.cuni.mff.been.resultsrepositoryng.RRTrigger)
	 */
	@Override
	public synchronized void createTrigger(RRTrigger trigger)
		...
		try {
			...
			DatasetDescriptor datasetDescriptor = getDatasetDescriptor(trigger.getAnalysis(), trigger.getDataset());
			if (!DatasetType.TRIGGER_ENABLED.equals(datasetDescriptor.getDatasetType())) {
				throw new ResultsRepositoryException("Cannot create trigger because dataset is not of type TRIGGER_ENABLED.");
			}			
			
			for (RRTrigger existingTrigger : triggers) {
				if (existingTrigger.getId().equals( trigger.getId()))
					throw new ResultsRepositoryException("Trigger with the same ID already exists.");
			}
			
			/* trigger will be only fired for new data */
			trigger.setLastProcessedSerial( serialNumberCounter.getCurrentSerial());
			...
			if (t != null) t.rollback();
			throw new ResultsRepositoryException("Error adding trigger to dataset \""+ trigger.getDataset() +"\".", e);
		} finally {
			if (session != null) session.close();
		}
	}
	...
	public synchronized void destroy() {
		...
		for (RRDataset dataset : datasets) {
			DatasetName name = new DatasetName(dataset.getAnalysis(), dataset.getName());
			SessionFactory sf = datasetSessionFactories.get( name );
			if (!sf.isClosed()) sf.close();
		}
		if (!mainSessionFactory.isClosed()) mainSessionFactory.close();
	}
	...
	/*
	 * (non-Javadoc)
	 * @see cz.cuni.mff.been.resultsrepositoryng.RRManagerInterface#deleteDataset(java.lang.String)
	 */
	@Override
	public synchronized void deleteDataset(String analysisName, String datasetName) throws ResultsRepositoryException, RemoteException {
		...
		try {
			...
			List<RRTrigger> triggers = this.getTriggers(session, analysisName, datasetName);
			for(RRTrigger trigger : triggers) {
			...
			try {
				...
				Query q = datasetSession.createQuery("delete from " + entityClasses.get(qname).getName());
				q.executeUpdate();
				
				datasetTransaction.commit();				
			} catch(Exception e) {
				if (datasetTransaction != null) datasetTransaction.rollback();
				throw new ResultsRepositoryException("Error deleting dataset's \""+ qname +"\" data.", e);
			} finally {
				if (datasetSession != null) datasetSession.close();
			}
			...
			if (t != null) t.rollback();
			throw new ResultsRepositoryException("Error deleting dataset \""+ qname +"\".", e);
		} finally {
			if (session != null) session.close();
		}
	}
	...
	/*
	 * (non-Javadoc)
	 * @see cz.cuni.mff.been.resultsrepositoryng.RRManagerInterface#deleteTriggers(java.lang.String, java.lang.String)
	 */
	@Override
	public synchronized void deleteTriggers(String analysis, String dataset, String evaluator)
			throws ResultsRepositoryException, RemoteException {
		...
		try {
			...
			if (dataset == null) {
				...
				for(RRDataset ds : datasets) 	
					if (ds.getName().equals(dataset)) {
						if ((analysis == null) || analysis.equals(ds.getAnalysis())) {
			...
			for(RRDataset ds : selectedDatasets) {
				List<RRTrigger> triggers = getTriggers(session, ds.getAnalysis(), ds.getName());
			
				for(RRTrigger trigger : triggers) {
					if ((evaluator == null) || trigger.getEvaluator().equals(evaluator)) {
						session.delete(trigger);
						firedTriggers.remove(trigger.getId());
						discardedTriggerEvents.remove(trigger.getId());
					}
	...
	throws ResultsRepositoryException, RemoteException, NoSuchElementException {
		...
		try {
			...
			if ( null == trigger ) {																// TODO: Don't throw unchecked!
				throw new NoSuchElementException( "Trigger with UUID " + triggerId.toString() + " does not exist." );
			}
			session.delete( trigger );
			firedTriggers.remove( trigger.getId() );
			discardedTriggerEvents.remove( trigger.getId() );
			...
			throw new ResultsRepositoryException( "Error deleting trigger.", exception );
		} finally {
			if ( null != session ) {
				session.close();
			}
	...
	/*
	 * (non-Javadoc)
	 * @see cz.cuni.mff.been.resultsrepositoryng.RRManagerInterface#getDatasetDescriptor(java.lang.String)
	 */
	@Override
	public synchronized DatasetDescriptor getDatasetDescriptor(String analysisName, String datasetName)
			throws ResultsRepositoryException, RemoteException {
		...
		if (result == null) {
			throw new ResultsRepositoryException("Dataset with name \""+qname+ "\" does not exist.");
		}
		return result;
	}
	...
	/*
	 * (non-Javadoc)
	 * @see cz.cuni.mff.been.resultsrepositoryng.RRManagerInterface#getDatasets()
	 */
	@Override
	public synchronized List<String> getDatasets(String analysisName) throws RemoteException, ResultsRepositoryException {
	...
	/*
	 * (non-Javadoc)
	 * @see cz.cuni.mff.been.resultsrepositoryng.RRManagerInterface#getAnalyses()
	 */
	@Override
	public synchronized List<String> getAnalyses() throws RemoteException, ResultsRepositoryException {
	...
	/*
	 * (non-Javadoc)
	 * @see cz.cuni.mff.been.resultsrepositoryng.RRManagerInterface#getTriggers(java.lang.String)
	 */
	@Override
	public synchronized List<RRTrigger> getTriggers(String analysis, String dataset)
			throws RemoteException, ResultsRepositoryException {
	...
	throws ResultsRepositoryException, RemoteException, NoSuchElementException {
		...
		try {
			...
			throw new ResultsRepositoryException( "Error retrieving trigger.", exception );
		} finally {
			if ( null != session ) {
				session.close();
			}
	 ...
	 * @param triggerId Trigger to look up.
	 ...
	 * @param session hibernate session to use
	 * @param analysisName analysis name filter or null (if null, condition will not be applied)
	 * @param datasetName dataset name filter or null (if null, condition will not be applied)
	 * @return list of triggers
	 */
	private List<RRTrigger> getTriggers(Session session, String analysisName, String datasetName) {
		Criteria query;
	 ...
	 * 
	 * Trigger's lastProcessedSerial is set to max(originalValue, lastProcessedSerial).
	 * 
	 ...
	 * @throws RemoteException
	 */
	public synchronized void notifyDataProcessed(UUID triggerId, long lastProcessedSerial) throws ResultsRepositoryException, RemoteException {
		...
		try {
			t = session.beginTransaction();
			RRTrigger trigger = (RRTrigger) session.get(RRTrigger.class, triggerId);
			...
			if (t != null) t.rollback();
			throw new ResultsRepositoryException("Error running triggers associated with notification event.", e);
		} finally {
			session.close();
		}
	}
	...
	/** 
	 * Ensures context classloader is set to entity class classloader
	 * (otherwise hibernate would fail to locate datasets' entity classes he needs).
	 */
	...
	/**
	 * Sets scheduler for evaluator.
	 * If scheduler not set, a default internal scheduler is used.
	 */
	...
	private class SerialNumberCounter {
		...
		/** 
		 * Raises current serial to given value,
		 * if current value is lower before.
		 * @param maxSerial new current serial
		 */
	...
	/**
	 * Class for representing qualified dataset's name.
	 * Qualified name consists of analysis name and dataset name.
	 * 
	 * @author Jan Tattermusch
	 *
	 */
	 ...
	 */
	@Override
	public synchronized RRTransaction getTransaction(String analysisName, String datasetName)
			throws RemoteException, ResultsRepositoryException {
		...
		DatasetDescriptor datasetDescriptor = getDatasetDescriptor(analysisName, datasetName);
		
		if (datasetDescriptor == null) {
			throw new ResultsRepositoryException("Dataset \""+qname+"\" does not exist.");	
		}
		
		if (!DatasetType.TRANSACTION_ENABLED.equals(datasetDescriptor.getDatasetType())) {
			throw new ResultsRepositoryException("Dataset \""+qname+"\" does not have transactions allowed. Check dataset type.");
		}
		...
		RRTransaction transaction = new RRTransactionImplementation(this, analysisName, datasetName, session, t);  
