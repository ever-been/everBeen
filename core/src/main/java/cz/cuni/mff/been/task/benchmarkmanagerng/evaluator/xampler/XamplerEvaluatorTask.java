...
/**
 * Xampler evaluator task evaluates data that come from tasks generated by
 * Xampler generator. Results are statistically analyzed and results are output
 * to a directory that conforms Regression Benchmarking Web specification (RBW
 * is a tool for presenting benchmark results on web).
 * 
...
public class XamplerEvaluatorTask extends EvaluatorTask {
	 ...
	 */
	private final File evalDir = new File(getWorkingDirectory(), "evaluation");
	 ...
	 * (non-Javadoc)
	 * 
	 * @see cz.cuni.mff.been.task.benchmarkmanagerng.evaluator.EvaluatorTask#
	...
	protected void doCheckRequiredProperties() throws TaskException {
		...
		if (destDir == null) {
			throw new TaskException("Property " + PROPERTY_DEST_DIR
					+ " not set.");
		}

		benchmarkName = getTaskProperty(PROPERTY_BENCHMARK_NAME);
		if (benchmarkName == null) {
			throw new TaskException("Property " + PROPERTY_BENCHMARK_NAME
					+ " not set.");
		}

		platformName = getTaskProperty(PROPERTY_PLATFORM_NAME);
		if (platformName == null) {
			throw new TaskException("Property " + PROPERTY_PLATFORM_NAME
					+ " not set.");
		}

		suiteName = getTaskProperty(PROPERTY_SUITE_NAME);
		if (suiteName == null) {
			throw new TaskException("Property " + PROPERTY_SUITE_NAME
					+ " not set.");
		}

		siteUrl = getTaskProperty(PROPERTY_SITE_URL);
		if (siteUrl == null) {
			throw new TaskException("Property " + PROPERTY_SITE_URL
					+ " not set.");
		}

		dependencyId = getTaskProperty(PROPERTY_DEPENDENCY_ID);
		if (dependencyId == null) {
			throw new TaskException("Property " + PROPERTY_DEPENDENCY_ID
					+ " not set.");
		}
	...
	protected void doRun() throws TaskException {
		...
		for (DataHandleTuple row : data) {
			String version;
			try {
				version = row.get(OMNIORB_TIMESTAMP_FIELD_NAME).getValue(
						String.class);
		...
		for (String version : updatedVersions) {
			...
			List<DataHandleTuple> currentVersionData = loadData(
					c,
					lowestDataSerial,
					highestDataSerial);

			GroupBy versionGroups = new GroupBy(
					currentVersionData,
					new String[] { XAMPLER_REVISION_FIELD_NAME,
							OMNIORB_TIMESTAMP_FIELD_NAME, SUITE_FIELD_NAME });

			for (DataHandleTuple versionKey : versionGroups.groups()) {
				Collection<DataHandleTuple> versionGroup = versionGroups
						.getGroup(versionKey);

				logInfo("Evaluating results for version: "
						+ versionKey.toString());
	 ...
	 * 
	 * @param groupData
	 *            xampler results rows for version to evaluate
	 * @param versionDir
	 *            directory to use when evaluating
	 * @throws TaskException
	 */
	private void evaluateVersion(
			Collection<DataHandleTuple> groupData,
			File versionDir) throws TaskException {
		try {
			purgeDir(evalDir);
			evalDir.mkdirs();
			...
			GroupBy buildGroups = new GroupBy(groupData, new String[] {
					XAMPLER_REVISION_FIELD_NAME, OMNIORB_TIMESTAMP_FIELD_NAME,
					SUITE_FIELD_NAME, BUILD_NUMBER_FIELD_NAME });

			for (DataHandleTuple buildKey : buildGroups.groups()) {
				Collection<DataHandleTuple> buildGroup = buildGroups
						.getGroup(buildKey);

				logInfo("Evaluating results for build: " + buildKey.toString());

				Integer buildNumber = buildKey.get(BUILD_NUMBER_FIELD_NAME)
	...
	private void evaluateBuild(
			Collection<DataHandleTuple> groupData,
			File buildDir) throws TaskException {
		try {
			buildDir.mkdirs();

			dumpResultsData(groupData, false); // we dont have to download
												// files, they're already
												// downloaded by evaluateVersion
			File resultFile = calculateStatistics(evalDir);
			Move.moveToDir(
					resultFile.getAbsolutePath(),
			...
			for (DataHandleTuple row : groupData) {
				Integer runNumber = row.get(RUN_NUMBER_FIELD_NAME).getValue(
						Integer.class);
				...
				/* move result file */
				File destArchive = new File(buildDir, runNumber.toString()
						+ ".bz2");

				BZip2.bzip(
						dataFile.getAbsolutePath(),
						destArchive.getAbsolutePath());
				dataFile.delete();
	...
	private void loadPluggableModules() throws TaskException {
		...
		try {
			csvFilesModule = CurrentTaskSingleton.getTaskHandle()
					.getPluggableModule(
							CSVFilesPluggableModule.class,
							"csvfiles",
							"1.0");

			RScriptingModule = CurrentTaskSingleton.getTaskHandle()
					.getPluggableModule(
							RScriptingPluggableModule.class,
							"rscripting",
							"1.0");
	...
	private void purgeDir(File dir) throws TaskException {
		try {
			...
			throw new TaskException("Error purging directory \""
					+ dir.getAbsolutePath() + "\"");
		}
	}
	 ...
	 *            data to dump
	 * @param downloadFiles
	 *            true if files referenced by file data handles should be
	 *            downloaded first
	 * @throws TaskException
	 */
	private void dumpResultsData(
			Collection<DataHandleTuple> data,
			boolean downloadFiles) throws TaskException {
		...
		try {
			...
			csv.writeCSVFile(
					...
					new String[] { XAMPLER_REVISION_FIELD_NAME,
							OMNIORB_TIMESTAMP_FIELD_NAME, SUITE_FIELD_NAME,
							BUILD_NUMBER_FIELD_NAME, RUN_NUMBER_FIELD_NAME,
							FILE_ID_FIELD_NAME },
					data,
					downloadFiles);
	 ...
	 * 
	 * @param homeDir
	 *            home directory (where to look for input files and where to
	 *            output the results).
	...
	private File calculateStatistics(File homeDir) throws TaskException {
		...
		try {
			int result = RScriptingModule.runRScript(
					scriptFile,
					homeDir.getAbsolutePath());
			if (result != 0) {
				throw new TaskException(
						"R script execution resulted in an error. Check "
								+ routFile.getAbsolutePath()
								+ " to see details.");
			}
	...
	/**
	 * Generates name of version destination directory (conforming to RBW
	 * specification)
	 * 
	 * @param key
	 *            version group key describing the version we are currently
	 *            processing.
	...
	private File getVersionDestDir(DataHandleTuple key) throws TaskException {
		try {
			String version = key.get(OMNIORB_TIMESTAMP_FIELD_NAME).getValue(
					String.class);
			String timestampString = formatTimestamp(version);

			String path = destDir + File.separator + timestampString
					+ File.separator + benchmarkName + File.separator
					+ platformName;
	...
	/**
	 * Constructs RBW parser's URL (used to notify RBW that new version data are
	 * available)
	 * 
	 * @param version
	 *            version that will be messaged to the parser
	 * @return parser's URL
	 */
	private String getParserUrl(String version) {
		return siteUrl + "?q=parser/" + dependencyId + "/"
				+ formatTimestamp(version);
	}
	 ...
	 * 
	 * @param version
	 *            version timestamp in format "YYYY-MM-DD HH:mm:ss"
	...
	private ResultRepositoryCSVUtils createCsvUtil() throws TaskException {
		try {
			return csvFilesModule.createResultsRepositoryCSVUtils(
					getResultsRepository().getFileStoreClient(),
					evalDir,
					SEPARATOR_CHAR,
					QUOTE_CHAR);
		} catch (RemoteException e) {
	...
	private void notifyParser(String url) throws TaskException {
		...
		try {
			...
			r = new BufferedReader(
					new InputStreamReader(parserUrl.openStream()));
