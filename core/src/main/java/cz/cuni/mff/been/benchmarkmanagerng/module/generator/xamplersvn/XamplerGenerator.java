...
public class XamplerGenerator extends GeneratorPluggableModule {
	...
	protected Collection<TaskDescriptor> doGenerate() throws GeneratorException {
		...
		planner.plan( getSvnVersions(), getAvailableSources(), getBuildLog(buildLogTransaction), getAvailableXamplerBinaries() ,getRunLog() );
	...
	private Set<SourceKey> getSvnVersions() throws GeneratorException  {
		...
		try {
			svnSources = versionProvider.getAvailableVersions(repository, versionsFromDate);
...
//	}
	 ...
	 */
	private TaskDescriptor downloadOmniorb(String timestamp, String treeParent, Condition downloadCondition) throws GeneratorException {
		...
		try {
			...
			throw new GeneratorException ("Invalid date encountered when creating the source download task.", e);
		}
		long dateLong = dateObject.getTime ();
		
		TaskDescriptor td = createTask("download-svn", downloadCondition, treeParent + "/omniORB SVN download");
	 ...
	 * @throws GeneratorException 
	 */
	private void initResultsRepositoryReference() throws RemoteException, GeneratorException {
