 ...
 */
package cz.cuni.mff.been.benchmarkmanagerng.module.generator.xamplersvn;
...
import cz.cuni.mff.been.benchmarkmanagerng.module.generator.xamplersvn.versionprovider.DefaultOmniorbSVNVersionProvider;
...
public class XamplerGenerator extends GeneratorPluggableModule {
	...
	protected Map<String, DatasetDescriptor> doCreateDatasets()
		...
		omniorbSrcDataset.put(
				SOURCE_TIMESTAMP_FIELD_NAME,
				DataType.STRING,
				true);
	...
	protected Collection<TaskDescriptor> doGenerate() throws GeneratorException {
		...
		if (readConfiguration(getConfiguration()) != null) {
			throw new GeneratorException(
					"Could not read configuration, configuration is not valid");
		}
		...
		planner.plan(
				getSvnVersions(),
				getAvailableSources(),
				getBuildLog(buildLogTransaction),
				getAvailableXamplerBinaries(),
				getRunLog());

		storePlanToRR(
				planner.getDownloadActivities(),
				planner.getBuildActivities(),
				planner.getRunActivities(),
				buildLogTransaction);
		...
		for (BuildActivity activity : planner.getBuildActivities()) {
			List<TaskDescriptor> activityTasks = getBuildActivityTasks(
					activity,
					null);
			/* add downloadXampler dependency, if needed */
			if (downloadXampler != null) {
		...
		for (RunActivity activity : planner.getRunActivities()) {
			List<TaskDescriptor> activityTasks = getRunActivityTasks(
					activity,
					null);
	...
	private RRTransaction getBuildLogTransaction() throws GeneratorException {
		try {
			return resultsRepositoryData.getTransaction(
					getBinaryAnalysisName(),
					PLANNED_BUILDS_DATASET_NAME);
		} catch (ResultsRepositoryException e) {
			e.printStackTrace();
			throw new GeneratorException(
					"Error obtaining transaction from RR.",
					e);
		} catch (RemoteException e) {
			e.printStackTrace();
			throw new GeneratorException(
	 ...
	 * 
	 * @param downloadActivities
	 *            download activities from planner
	 * @param buildActivities
	 *            build activities from planner
	 * @param runActivities
	 *            run activities from planner
	 * @param buildLogTransaction
	 * @throws GeneratorException
	 */
	private void storePlanToRR(
			List<DownloadActivity> downloadActivities,
			List<BuildActivity> buildActivities,
			List<RunActivity> runActivities,
			RRTransaction buildLogTransaction) throws GeneratorException {
		...
		try {
			...
			throw new GeneratorException(
					"Error storing execution plan to RR.",
					e);
		} catch (ResultsRepositoryException e) {
			throw new GeneratorException(
					"Error storing execution plan to RR.",
	 ...
	 * 
	 * @param ba
	 *            build activity to store
	 * @param buildLogTransaction
	 * @throws GeneratorException
	 */
	private void storeBuildActivity(
			BuildActivity ba,
			RRTransaction buildLogTransaction) throws GeneratorException {
		try {
			...
			t.set(
					OMNIORB_TIMESTAMP_FIELD_NAME,
					DataHandle.create(DataType.STRING, ba.getVersion()));
			t.set(
					BUILD_NUMBER_FIELD_NAME,
					DataHandle.create(DataType.INT, ba.getBuildNumber()));
			buildLogTransaction.saveData(t);
		} catch (RemoteException e) {
			throw new GeneratorException(
					"Error storing build activity to planned builds dataset.",
					e);
		} catch (ResultsRepositoryException e) {
			throw new GeneratorException(
					"Error storing build activity to planned builds dataset.",
	...
	private void storeRunActivity(RunActivity ra) throws GeneratorException {
		try {
			for (String suite : ra.getSuites()) {
				...
				t.set(
						OMNIORB_TIMESTAMP_FIELD_NAME,
						DataHandle.create(DataType.STRING, ra.getVersion()));
				t.set(
						BUILD_NUMBER_FIELD_NAME,
						DataHandle.create(DataType.INT, ra.getBuildNumber()));
				t.set(
						RUN_NUMBER_FIELD_NAME,
						DataHandle.create(DataType.INT, ra.getRunNumber()));
				t.set(
						SUITE_FIELD_NAME,
						DataHandle.create(DataType.STRING, suite));
				resultsRepositoryData.saveData(
						getAnalysis().getName(),
						PLANNED_RUNS_DATASET_NAME,
						t);
			}
		} catch (RemoteException e) {
			throw new GeneratorException(
					"Error storing run activity to planned runs dataset.",
					e);
		} catch (ResultsRepositoryException e) {
			throw new GeneratorException(
					"Error storing run activity to planned runs dataset.",
	...
	/**
	 * Retrieves set of OmniORB versions available in SVN.
	 * 
	 * @return set of OmniORB versions
	 * @throws GeneratorException
	 */
	private Set<SourceKey> getSvnVersions() throws GeneratorException {

		DefaultOmniorbSVNVersionProvider versionProvider = new DefaultOmniorbSVNVersionProvider();

		String repository = omniorbSvnUrl;

		Set<SourceKey> svnSources;
		try {
			svnSources = versionProvider.getAvailableVersions(
					repository,
					versionsFromDate);
		} catch (AnalysisException e) {
			e.printStackTrace();
			throw new GeneratorException(
					"Error getting available version list from version control.",
					e);
		}
	 ...
	 * 
	 * @param xamplerRevision
	 *            check for this xampler revision
	 * @return true if xampler sources are available
	 * @throws GeneratorException
	 */
	private boolean isXamplerAvailable(Long xamplerRevision)
			throws GeneratorException {
		List<DataHandleTuple> xamplerSources = getDatasetData(
				getSourcesAnalysisName(),
				XAMPLER_SRC_DATASET_NAME,
				null);
		for (DataHandleTuple xamplerSource : xamplerSources) {
			try {
				if (xamplerRevision.equals(xamplerSource.get(
						SOURCE_REVISION_FIELD_NAME).getValue(Long.class))) {
					return true;
				}
			} catch (DataHandleException e) {
				throw new GeneratorException(
						"Error checking whether Xampler sources are available.",
						e);
			}
		}
		return false;
	...
	/**
	 * Factory method for creating planner that will be used for generating
	 * tasks.
	 * 
	...
	private DBRPlanner createPlanner() {
		...
		return new SimpleDownloadBuildRunPlanner(
				suites,
				workloadUnits,
				downloadCost,
				buildCost,
	 ...
	 * 
	 * @param activity
	 *            run activity to perform
	 * @param treeParent
	 *            tree prefix to use for generated tasks (if null, default tree
	 *            prefix will be used)
	 * @return task tasks to perform
	 */
	private List<TaskDescriptor> getRunActivityTasks(
		...
		if (treeParent == null) {
			treeParent = "run-" + omniorbTimestamp + "-" + buildNumber + "-"
					+ runNumber;
		}
		...
		if (Debug.isDebugModeOn()) {
			CurrentTaskSingleton
					.getTaskHandle()
					.logInfo(
							"Generator task in debug mode. Execute tasks will be not marked as TaskExclusivity.EXCLUSIVE.");
			exclusiveRuns = false;
		}
		...
		if (exclusiveRuns && simulate) {
			CurrentTaskSingleton
					.getTaskHandle()
					.logInfo(
							"Analysis in simulation mode. Execute tasks will be not marked as TaskExclusivity.EXCLUSIVE.");
			exclusiveRuns = false;
		}
		...
		for (String suiteKey : activity.getSuites()) {
			...
			if (subsuiteDirName != null) {
				suitePath = suiteDirName + File.separator + xamplerSuiteSubdir
						+ File.separator + subsuiteDirName;
			} else {
				suitePath = suiteDirName + File.separator + xamplerSuiteSubdir;
			}

			if (!runOnSingleHost) {
				TaskDescriptor deployXamplerServer = deployXampler(
						xamplerSvnRevision,
						omniorbTimestamp,
						buildNumber,
						serverRsl,
				...
				TaskDescriptor deployXamplerClient = deployXampler(
						xamplerSvnRevision,
						omniorbTimestamp,
						buildNumber,
						clientRsl,
				...
				TaskDescriptor executeXamplerServer = executeXamplerServer(
						deployXamplerServer.getTaskId(),
						suitePath,
						xamplerServerParams,
						treeParent);
				success(executeXamplerServer, deployXamplerServer.getTaskId());
				tasks.add(executeXamplerServer);
				executeXamplerServer.getHostRuntimes().setAsTask(
						deployXamplerServer.getTaskId()); // HRs should always
															// be set.
				if (exclusiveRuns) {
					executeXamplerServer
							.setExclusive(TaskExclusivity.EXCLUSIVE);
				}

				TaskDescriptor executeXamplerClient = executeXamplerClient(
						deployXamplerClient.getTaskId(),
						suitePath,
				...
				executeXamplerClient.getHostRuntimes().setAsTask(
						deployXamplerClient.getTaskId()); // HRs shold always be
															// set.

				TaskDescriptor collectXamplerResults = collectXamplerResults(
				...
				collectXamplerResults.getHostRuntimes().setAsTask(
						executeXamplerClient.getTaskId());// HRs should always
															// be set.

			} else {
				Condition hostRsl = clientRsl;

				TaskDescriptor deployXampler = deployXampler(
						xamplerSvnRevision,
						omniorbTimestamp,
						buildNumber,
						hostRsl,
				...
				TaskDescriptor executeXampler = executeXamplerClientAndServer(
						deployXampler.getTaskId(),
						suitePath,
						xamplerClientParams,
						xamplerServerParams,
						treeParent);
				...
				executeXampler.getHostRuntimes().setAsTask(
						deployXampler.getTaskId()); // HRs should always be set.
				if (exclusiveRuns) {
				...
				tasks.add(collectXamplerResults);
				collectXamplerResults.getHostRuntimes().setAsTask(
						executeXampler.getTaskId()); // HRs should always be
														// set.
			}
		}
	...
	private List<TaskDescriptor> getBuildActivityTasks(
		...
		TaskDescriptor buildOmniorb = buildOmniorb(
				omniorbTimestamp,
				buildNumber,
				buildRsl,
				treeParent);
		...
		TaskDescriptor buildXampler = buildXampler(
				xamplerSvnRevision,
				omniorbTimestamp,
				buildNumber,
				buildRsl,
		...
		for (RunActivity depActivity : activity.getRunActivities()) {
			List<TaskDescriptor> dependentTasks = getRunActivityTasks(
					depActivity,
					treeParent);
	...
	private List<TaskDescriptor> getDownloadActivityTasks(
			DownloadActivity activity) throws GeneratorException {

		List<TaskDescriptor> tasks = new ArrayList<TaskDescriptor>();

		String treeParent = "download-build-run-" + activity.getVersion();

		TaskDescriptor downloadOmniorb = downloadOmniorb(
				activity.getVersion(),
				treeParent,
				downloadRsl);
		...
		for (BuildActivity depActivity : activity.getBuildActivities()) {
			List<TaskDescriptor> dependentTasks = getBuildActivityTasks(
					depActivity,
					treeParent);
	 ...
	 *            name of dataset
	 * @param condition
	 *            condition that has to be satfisfied
	 * @return data of dataset
	 * @throws GeneratorException
	 */
	private List<DataHandleTuple> getDatasetData(
			String analysisName,
			String datasetName,
		...
		try {
			data = resultsRepositoryData.loadData(
					analysisName,
					datasetName,
					condition,
					null,
	 ...
	 * @throws GeneratorException
	 */
	private List<DataHandleTuple> getDatasetData(String datasetName)
			throws GeneratorException {
	...
	private Set<SourceKey> getAvailableSources() throws GeneratorException {
		...
		for (DataHandleTuple tuple : getDatasetData(
				getSourcesAnalysisName(),
				OMNIORB_SRC_DATASET_NAME,
				null)) {
			String timestamp;
			try {
				timestamp = tuple.get(SOURCE_TIMESTAMP_FIELD_NAME)
						.getValue(String.class).toString();

			} catch (DataHandleException e) {
				throw new GeneratorException(
						"Error retrieving available sources list.",
						e);
			}
	 ...
	 * @throws GeneratorException
	 */
	private MultiMap<BuildKey, BuildProps> getBuildLog(
			RRTransaction buildLogTransaction) throws GeneratorException {
		try {
			...
			for (DataHandleTuple tuple : buildLogTransaction.loadData(null)) {
				...
				Integer buildNumber;
				try {
					timestamp = tuple.get(OMNIORB_TIMESTAMP_FIELD_NAME)
							.getValue(String.class).toString();
					buildNumber = tuple.get(BUILD_NUMBER_FIELD_NAME).getValue(
							Integer.class);
				} catch (DataHandleException e) {
					throw new GeneratorException(
							"Error retrieving available sources list.",
			...
			throw new GeneratorException(
					"Error obtaining planner build log data",
					e);
		} catch (RemoteException e) {
			throw new GeneratorException(
					"Error obtaining planner build log data",
	...
	/**
	 * Retrieves list of available xampler binaries (that were successfully
	 * built before).
	 * 
	 * @return list of available binaries
	 * @throws GeneratorException
	 */
	private MultiMap<BuildKey, BuildProps> getAvailableXamplerBinaries()
			throws GeneratorException {
		MultiMap<BuildKey, BuildProps> result = new MultiHashMap<BuildKey, BuildProps>();

		cz.cuni.mff.been.resultsrepositoryng.condition.Condition condition = Restrictions
				.isNotNull(FILE_ID_FIELD_NAME);

		for (DataHandleTuple tuple : getDatasetData(
				getBinaryAnalysisName(),
				XAMPLER_BIN_DATASET_NAME,
				condition)) {
			String timestamp;
			Integer buildNumber;
			try {
	...
	// /**
	// * Retrieves list of available omniorb binaries (that were successfully
	// built before).
	// * @return list of available binaries
	// * @throws GeneratorException
	// */
	// private MultiMap<BuildKey, BuildProps> getAvailableOmniorbBinaries()
	// throws GeneratorException {
	// MultiMap<BuildKey, BuildProps> result = new MultiHashMap<BuildKey,
	// BuildProps>();
	//
	// cz.cuni.mff.been.resultsrepositoryng.condition.Condition condition =
	// Restrictions.isNotNull(FILE_ID_FIELD_NAME);
	//
	// for(DataHandleTuple tuple : getDatasetData( getBinaryAnalysisName(),
	// OMNIORB_BIN_DATASET_NAME, condition )) {
	// String timestamp;
	// Integer buildNumber;
	// try {
	// timestamp =
	// tuple.get(OMNIORB_TIMESTAMP_FIELD_NAME).getValue(String.class).toString();
	// buildNumber = tuple.get(BUILD_NUMBER_FIELD_NAME).getValue(Integer.class);
	// } catch (DataHandleException e) {
	// throw new GeneratorException("Error retrieving available sources list.",
	// e);
	// }
	//
	// result.put(new BuildKey(timestamp), new BuildProps(buildNumber));
	...
	private MultiMap<RunKey, RunProps> getRunLog() throws GeneratorException {
		...
		for (DataHandleTuple tuple : getDatasetData(PLANNED_RUNS_DATASET_NAME)) {
			...
			try {
				...
				buildNumber = tuple.get(BUILD_NUMBER_FIELD_NAME).getValue(
						Integer.class);
				runNumber = tuple.get(RUN_NUMBER_FIELD_NAME).getValue(
						Integer.class);
	 ...
	 * 
	 * @param timestamp
	 *            version of omniorb to download
	 * @param treeParent
	 *            tree prefix to use in task's tree address
	 * @param downloadCondition
	 *            RSL condition for task's host
	 * @return task's taskdescriptor
	 */
	private TaskDescriptor downloadOmniorb(
			String timestamp,
			String treeParent,
			Condition downloadCondition) throws GeneratorException {

		DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		dateFormat.setTimeZone(TimeZone.getTimeZone("GMT"));
		Date dateObject;
		try {
			dateObject = dateFormat.parse(timestamp);
		} catch (java.text.ParseException e) {
			throw new GeneratorException(
					"Invalid date encountered when creating the source download task.",
					e);
		}
		long dateLong = dateObject.getTime();

		TaskDescriptor td = createTask(
				"download-svn",
				downloadCondition,
				treeParent + "/omniORB SVN download");
		TaskDescriptorHelper.addTaskProperties(
				td,
				Pair.pair("url", omniorbSvnUrl),
				Pair.pair("timestamp", dateLong),
				Pair.pair("rr.analysis", getSourcesAnalysisName()),
				Pair.pair("rr.dataset", OMNIORB_SRC_DATASET_NAME));
		if (simulate) {
			TaskDescriptorHelper.addTaskProperties(
					td,
					Pair.pair("simulate", "yes"));
		}
		return td;
	}
	 ...
	 * 
	 * @param revision
	 *            version of Xampler to download (SVN revision)
	 * @param downloadCondition
	 *            RSL condition for task's host
	 * @return task's taskdescriptor
	 */
	private TaskDescriptor downloadXampler(
			Long revision,
			Condition downloadCondition) {
		TaskDescriptor td = createTask(
				"download-svn",
				downloadCondition,
				"Xampler SVN donwload");
		TaskDescriptorHelper.addTaskProperties(
				td,
	 ...
	 * 
	 * @param timestamp
	 *            version of omniORB to build
	 * @param buildNumber
	 *            build number of build
	 * @param buildCondition
	 *            RSL condition for task's host
	 * @param treeParent
	 *            tree prefix that task should use
	 * @return task's taskdesriptor
	 */
	private TaskDescriptor buildOmniorb(
			String timestamp,
			Integer buildNumber,
	 ...
	 * 
	 * @param xamplerRevision
	 *            version of Xampler to build
	 * @param omniorbTimestamp
	 *            version of omniORB to build
	 * @param buildNumber
	 *            build number of build
	 * @param buildCondition
	 ...
	 * @return task's taskdesriptor
	 */
	private TaskDescriptor buildXampler(
			Long xamplerRevision,
			String omniorbTimestamp,
			Integer buildNumber,
			Condition buildCondition,
			String treeParent) {
		TaskDescriptor td = createTask(
	 ...
	 *            build number
	 * @param hostCondition
	 *            condition to which host should be deployed
	 * @param treeParent
	 *            tree prefix for task
	 * @return task's taskdescriptor
	 */
	private TaskDescriptor deployXampler(
			Long xamplerRevision,
			String omniorbTimestamp,
			Integer buildNumber,
	 ...
	 * 
	 * @param deployTid
	 *            task ID of corresponding deploy task
	 * @param suitePath
	 *            path to suite that should be run
	 * @param isServer
	 *            task's role (null means singlehost mode)
	 * @param treeParent
	 *            tree prefix for task's tree address
	 * @return task's taskdescriptor
	 */
	private TaskDescriptor executeXampler(
			String deployTid,
			String suitePath,
			Boolean isServer,
			String treeParent) {
		TaskDescriptor td = createTask(
				...
				treeParent
						+ (isServer != null ? (isServer ? "/Xampler server execute"
								: "/Xampler client execute")
								: "/Xampler execute"));

		String omniorbRoot = "${" + deployTid + ":workingDirectory}"
				+ File.separator + OMNIORB_DIR;
		String xamplerRoot = "${" + deployTid + ":workingDirectory}"
				+ File.separator + XAMPLER_DIR;
	 ...
	 * 
	 * @param clientExecuteTid
	 *            task ID of client execute task
	 * @param xamplerRevision
	 *            version of Xampler
	 * @param omniorbTimestamp
	 *            version of omniORB
	 ...
	 * @return task's taskdescriptor
	 */
	private TaskDescriptor collectXamplerResults(
			String clientExecuteTid,
			Long xamplerRevision,
			String omniorbTimestamp,
			Integer buildNumber,
			Integer runNumber,
			...
			String treeParent) {
		TaskDescriptor td = createTask(
				"xampler-collect-results",
				Analysis.DEFAULT_HOST_RSL,
				treeParent + "/Xampler collect results");
	 ...
	 *            deploy task's ID
	 * @param suitePath
	 *            path to suite that will be executed
	 * @param params
	 *            xampler server parameters
	 * @param treeParent
	 *            tree prefix for task's tree address
	 * @return task's taskdesriptor
	 */
	private TaskDescriptor executeXamplerServer(
			String deployTid,
			String suitePath,
			String params,
			String treeParent) {
		TaskDescriptor td = executeXampler(
				deployTid,
				suitePath,
				true,
				treeParent);
	 ...
	 *            path to suite that will be executed
	 * @param serverTid
	 *            execute server task's ID
	 * @param params
	 *            xampler client parameters
	 * @param treeParent
	 *            tree prefix for task's tree address
	 * @return task's taskdesriptor
	 */
	private TaskDescriptor executeXamplerClient(
			String deployTid,
			String suitePath,
			String serverTid,
			String params,
	 ...
	 * @return task's taskdesriptor
	 */
	private TaskDescriptor executeXamplerClientAndServer(
			String deployTid,
			String suitePath,
			String clientParams,
			String serverParams,
	 ...
	 * @throws GeneratorException
	 */
	private void initResultsRepositoryReference() throws RemoteException,
			GeneratorException {
	...
	/**
	 * For suite key (that can contain both name of suite and subsuite)return
	 * appropriate suite directory name
	 * 
	 * @param suiteKey
	 *            suite key (identifier)
	...
	/**
	 * For suite key (that can contain both name of suite and subsuite) return
	 * appropriate subsuite directory name
	 * 
	 * @param suiteKey
	 *            suite key (identifier)
	 ...
	 *            string to parse
	 * @param fieldName
	 *            field name that will appear in error message
	 * @param errors
	...
	Integer parseInt(String s, String fieldName, List<String> errors) {
		try {
			...
			errors.add("Error parsing field " + fieldName + ": "
					+ e.getMessage());
			return null;
		}
	}
	 ...
	 */
	private Condition parseRSLCondition(
			String string,
			String fieldName,
			List<String> errors,
			boolean allowEmpty) {
	...
	private Set<String> prepareSuiteSet(
			String[] suiteIds,
			Pattern subsuitePattern) {
	...
	/**
	 * @return map that contains list of subsuites for every suite that has some
	 *         subsuites
	 */
	private Map<String, String[]> getSubsuiteMap() {
		...
		result.put("mta", new String[] { "Boolean_1024_IN",
				"LongDouble_64_RETURN", "ULongLong_1024_OUT",
				"Boolean_1024_INOUT", "LongLong_1024_IN",
				"ULongLong_1024_RETURN", "Boolean_1024_OUT",
				"LongLong_1024_INOUT", "ULongLong_128_IN",
				"Boolean_1024_RETURN", "LongLong_1024_OUT",
				"ULongLong_128_INOUT", "Double_1024_IN",
				"LongLong_1024_RETURN", "ULongLong_128_OUT",
		...
		result.put("mts", new String[] { "Boolean_IN", "Char_INOUT",
				"Long_OUT", "ULongLong_INOUT", "Boolean_INOUT", "Char_OUT",
				"Long_RETURN", "ULongLong_OUT", "Boolean_OUT", "Char_RETURN",
				"Octet_IN", "ULongLong_RETURN", "Boolean_RETURN",
				"LongDouble_IN", "Octet_INOUT", "ULong_OUT", "Double_IN",
				"LongDouble_INOUT", "Octet_OUT", "ULong_RETURN",
	...
	private Set<SourceKey> parseVersionBlacklist(String string)
	...
	/**
	 * Reads configuration from given object and validates it. Returns list of
	 * error messages. Read configuration is stored into this object's fields
	 * 
	 * @param configuration
	 *            configuration
	...
	private List<String> readConfiguration(Configuration configuration) {
		...
		runBuildRatio = parseFloat(
				configuration.get("run.build.ratio", 0),
				"run.build.ratio",
				errors);
		maxNumberOfRuns = parseInt(
				configuration.get("max.runs", 0),
				"max.runs",
				errors);
		maxNumberOfBuilds = parseInt(
				configuration.get("max.builds", 0),
				"max.builds",
				errors);

		workloadUnits = parseInt(
				configuration.get("workload.units", 0),
				"workload.units",
				errors);
		downloadCost = parseInt(
				configuration.get("download.cost", 0),
				"download.cost",
				errors);
		buildCost = parseInt(
				configuration.get("build.cost", 0),
				"build.cost",
				errors);
		runCost = parseInt(configuration.get("run.cost", 0), "run.cost", errors);

		omniorbSvnUrl = configuration.get("omniorb.svn.url", 0);

		xamplerSvnUrl = configuration.get("xampler.svn.url", 0);
		xamplerSvnRevision = parseLong(
				configuration.get("xampler.svn.revision", 0),
				"xampler.svn.revision",
				errors);
		...
		downloadRsl = parseRSLCondition(
				configuration.get("downloadrsl", 0),
				"downloadrsl",
				errors,
				false);
		clientRsl = parseRSLCondition(
				configuration.get("clientrsl", 0),
				"clientrsl",
				errors,
				false);
		buildRsl = parseRSLCondition(
				configuration.get("buildrsl", 0),
				"buildrsl",
				errors,
				false);
		/* if null, execute will be performed on a single host */
		serverRsl = parseRSLCondition(
				configuration.get("serverrsl", 0),
				"serverrsl",
				errors,
				true);
		...
		sourcesRepositoryAnalysisSafeName = configuration.get(
				"sources.repository.analysis",
				0);
		...
		binaryRepositoryAnalysisSafeName = configuration.get(
				"binaries.repository.analysis",
				0);

		/* versions.from.date */
		versionsFromDate = null;
		String versionsFromDateString = configuration.get(
				"versions.from.date",
				0);
		if (versionsFromDateString != null && !versionsFromDateString.isEmpty()) {
			...
			try {
				...
				errors.add("Unable to parse date \"" + versionsFromDateString
						+ "\"" + e.getMessage());
			}
		}
		...
		/* version blacklist */
		try {
			versionBlacklist = parseVersionBlacklist(configuration.get(
					"version.blacklist",
					0));
		} catch (java.text.ParseException e) {
		...
		try {
			...
			errors.add("Unable to parse subsuite selection regex \""
					+ subsuiteRegexString + "\"" + e.getMessage());
		}
	...
	/**
	 * 
	 * @return safe name of analysis from where to get xampler and omniorb
	 *         sources
	 */
