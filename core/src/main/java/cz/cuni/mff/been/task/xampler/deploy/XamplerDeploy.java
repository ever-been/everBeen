...
public class XamplerDeploy extends Job {
	...
	protected void run() throws TaskException {
		...
		try {
			Untar.untar(
					...
					"gzip");
			Untar.untar(
					xamplerArchive.getAbsolutePath(),
					xamplerRoot.getAbsolutePath(),
					"gzip");
	...
	protected void checkRequiredProperties() throws TaskException {
		checkRequiredProperties(new String[] { RR_OMNIORB_ANALYSIS_NAME,
				RR_OMNIORB_DATASET_NAME, RR_XAMPLER_ANALYSIS_NAME,
				RR_XAMPLER_DATASET_NAME, OMNIORB_TIMESTAMP, XAMPLER_REVISION,
				BUILD_NUMBER, OMNIORB_DIR, XAMPLER_DIR });
