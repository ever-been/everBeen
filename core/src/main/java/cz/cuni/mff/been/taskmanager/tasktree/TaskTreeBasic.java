...
abstract class TaskTreeBasic implements TaskTreeItem, Serializable {
	...
	/**
	 * Flags implemented inside an enum can't reimplement Comparable. This
	 * solves the problem.
	 */
	private static final Comparator<TaskTreeFlag> FLAG_COMPARATOR = new Comparator<TaskTreeFlag>() {
		@Override
	 ...
	 * @param parentAddress
	 *            Address of the parent tree node.
	 */
	TaskTreeBasic(TaskTree tree, TaskTreeAddressBody address,
	...
	@Override
	public Collection<TaskTreeAddressBody> getChildren()
	...
	public final TaskTreeFlag[] getFlags() {
		...
		if (null == result) {
			...
			result = flags.values().toArray(new TaskTreeFlag[flags.size()]); // OK,
																				// read-only
																				// access.
	 ...
	 * 
	 * @param flag
	 *            The flag for which the ordinal value should be found.
	...
	public final Pair<TaskTreeFlag, TreeFlagValue>[] getFlagValues() {
		...
		if (null == result) {
			int i;
			acquireLock();
			result = (Pair<TaskTreeFlag, TreeFlagValue>[]) Array.newInstance(
					Pair.class,
					flags.size());
			i = 0;
			for (Entry<TaskTreeFlag, TreeFlagValue> flag : flags.entrySet()) { // OK,
																				// read-only
																				// access.
	...
	/**
	 * Sets a tree flag to the given value. Locks must be handled by the caller
	 * here, since they need to take the tree hierarchy into account!
	 * 
	 * @param flag
	 *            Type of the flag to set.
	 * @param value
	 *            An enum representing the flag's value.
	 * @return The previous value of the flag, null if set the first time.
	 * @throws TreeFlagException
	 *             When a nonexistent flag is set null.
	 */
	TreeFlagValue setFlag(TaskTreeFlag flag, TreeFlagValue value)
			throws TreeFlagException {
		TreeFlagValue entry;
		...
		if (value == flag.getDefaultValue()) { // YUP, instance does matter.
			...
			if (null == entry) {
				throw new TreeFlagException(
						"Removing a nonexistent flag.",
						flag); // No change, no invalidation.
			}
		} else {
	...
	/**
	 * Acquires an advisory lock on the current task tree item. No debugging, no
	 * deadlock checks. Should be used with caution and called only by the
	 * TaskTree class.
	 */
	...
	/**
	 * Releases the advisory lock on the current task tree item. Should be used
	 * with caution and called only by the TaskTree class.
	 */
