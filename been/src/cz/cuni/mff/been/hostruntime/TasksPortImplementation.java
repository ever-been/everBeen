 ...
 * @author David Majda
 */
public class TasksPortImplementation extends UnicastRemoteObject implements TasksPortInterface {
	...
	private HostRuntimesPortInterface hostRuntimesPort;
	 ...
	 */
	public TasksPortImplementation(TaskImplementation task) throws RemoteException {
		this.task = task;
	...
	public Properties getTaskProperties() throws RemoteException {
		...
		return VariableReplacer.replace( task.getTaskDescriptor(), valueProvider );					// Returns empty when not set.
	}
	
	public Map<String, Serializable> getTaskPropertyObjects() throws RemoteException {
	...
	public void runTasks(TaskDescriptor[] taskDescriptor) throws RemoteException {
		task.getHostRuntime().getTaskManager().runTask(taskDescriptor);
	}
	...
	public void registerEventListener(HostRuntimeRegistrationListener listener) throws RemoteException {
		task.getHostRuntime().getTaskManager().registerEventListener(listener);
	}

	public void unregisterEventListener(HostRuntimeRegistrationListener listener) throws RemoteException {
	...
	public Serializable checkPointWait(String contextID, String taskID, String name,
	...
	public TaskEntry getTaskById(String taskId, String contextId) throws RemoteException, IllegalArgumentException {
	...
	public void extractPackage(String packageName, String packageVersion, String path, PackageType packageType) throws RemoteException {
		try {
			task.getHostRuntime().getPackageCacheManager().extractPackage(packageName, packageVersion, path, packageType);
		} catch (Exception ex) {
