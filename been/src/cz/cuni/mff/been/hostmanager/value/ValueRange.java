 ...
 */
public class ValueRange< T extends ValueBasicInterface< T > > 
	implements ValueCompoundInterface< T >, Serializable, XMLSerializableInterface {
	...
	/**
	 * Allocate {@code ValueRange} which represents the whole interval. This constructor
	 ...
	 *        positive infinity.
	 */
	public ValueRange(T min, T max, boolean leftOpen, boolean rightOpen, ValueType elementType) {
	...
	public void parseXMLNode(Node node) throws InputParseException {
		...
		if (null == elementType) {
			...
			if (XMLHelper.hasSubNode("min", node)) {
				...
				try {
					valueMin = (T) Class.forName(elementType.NAME).newInstance();
				} catch (Exception e) {
			...
			if (XMLHelper.hasSubNode("max", node)) {
				...
				try {
					valueMax = (T) Class.forName(elementType.NAME).newInstance();
				} catch (Exception e) {
	...
	public Element exportAsElement(Document document) {
		...
		element.setAttribute("subtype", null == elementType ? EMPTY_CLASS_NAME : elementType.NAME);
