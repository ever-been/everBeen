...
import cz.cuni.mff.been.webinterface.LogUtils;

/**
 * A command line interface component that corresponds to the hosts listing screen.
 * 
...
public final class HostsModule extends CommandLineModule {
	...
	public enum Errors implements Message {
		...
		/** When the Host Manager is not responding (RemoteException, IOException). */
		CONN_HM( "Could not contact the Host Manager." ),
		
		/** When the Load Server is not responding (RemoteException, IOException). */
		...
		/** Either the host name is invalid, or no logs have been generated so far. (!!!) */
		...
		/** When an unknown/unexpected NameValuePair is found in a software alias. */
		INTG_NVP( "Illegal item in restriction or properties data (name-value pair)." ),
		
		/** When an unknown/unexpected NameValuePair is found in a software alias. */
		INTG_PROPS( "Illegal item in restriction or properties XML data." ),
		
		/** When the restriction cannot be cast to ObjectRestriction or RSLRestriction. */
		...
		MALF_RSL_APP( "The RSL expression for appliaction selection is malformed." ),
		
		/** When the RSLRestriction refuses a RSL query. */
		MALF_RSL_OS( "The RSL expression for operating system selection is malformed." ),
		...
		/** When object unmarshaling fails due to validation and data format issues. */
	...
	static {
		...
		actions.put(
			"list",
			new CommandLineAction< HostsModule >() {
				...
				public void handle(
					...
					try {
						hostInfos = module.hostManagerReference.get().getHostInfos();
						if ( null == pattern ) {
							...
							for ( HostInfoInterface hostInfo : hostInfos ) {
								if ( cpattern.matcher( hostInfo.getHostName() ).matches() ) {
									writer.sendLinePlain( hostInfo );
								}
							}						
						}
			...
			new CommandLineAction< HostsModule >() {
				...
				public void handle(
					...
					try {
						writer.sendLine(
							hostName,
							loadServer.getHostStatus( hostName ),									// Atomicity not guaranteed!
							loadServer.getLastSample( hostName )									// <<< HERE! A different host?
						);
		...
		actions.put(
			...
			new CommandLineAction< HostsModule >() {
				...
				private final Set< String > flags = stringSet(
					"system", "cpu", "storage", "network", "history"								// Keep in sync with Writer!
				);
				...
				public void handle(
					...
					if ( null == date ) {
						...
						try {
							...
							throw new ModuleSpecificException( Errors.FAIL_HOST_QUERY, exception );
						} catch ( ValueNotFoundException exception ) {
							throw new ModuleSpecificException(
					...
					try {
						...
						if ( request.getFlag( "system" ) ) {
							writer.sendLineSystem( hostInfo.getOperatingSystem(), makeGap );
							makeGap = true;
						} else {
							makeGap = false;
						}
					...
					try {
						...
						if ( null == host ) {
							if ( request.getFlag( "all" ) ) {
								...
								for ( OperationHandle handle : handles.values() ) {					// No output. No active waiting!
									hostManager.removeOperationStatus( handle );
								}
							} else {
								throw new ModuleSpecificException( Errors.REQD_ALL_HOST );
							}
						} else {
							if ( request.getFlag( "all" ) ) {
								throw new ModuleSpecificException( Errors.EXCL_ALL_HOST );
							} else {
								OperationHandle handle;
								
								hostManager = module.hostManagerReference.get();
								handle = hostManager.refreshHost( host );
								hostManager.removeOperationStatus( handle );						// No output. No active waiting!
							}
						}
					} catch ( RemoteException exception ) {
						throw new ModuleSpecificException( Errors.CONN_HM, exception );
						...
						throw new ModuleSpecificException( Errors.UNKN_HOST, exception );
					} catch ( ValueNotFoundException exception ) {
						throw new ModuleSpecificException( Errors.UNKN_HOST, exception );
					}
				}
		...
		actions.put(
			"tasks",
			new CommandLineAction< HostsModule >() {
				private final Set< String > allParameters = stringSet( "host", "pattern" );
				private final Set< String > requiredParameters = stringSet( "host" );
				...
				public void handle(
					...
					try {
						...
						if ( null == pattern ) {
							...
							if ( request.getFlag( "desc" ) ) {
								for ( TaskEntry task : tasks ) {
									if ( cpattern.matcher( task.getTaskName() ).matches() ) {
										writer.sendLineDescr( task );
									}
								}
							} else {
			...
			new CommandLineAction< HostsModule >() {
				...
				public void handle(
					...
					try {
						...
						throw new ModuleSpecificException( Errors.FAIL_LOG_QUERY, exception );
					} catch ( RemoteException exception ) {
						throw new ModuleSpecificException( Errors.CONN_TM, exception );
					}

					if ( records.length == 0 ) {
						throw new ModuleSpecificException( Errors.UNKN_HOST_LOGS, hostName );
					}
		...
		actions.put(
			...
			new CommandLineAction< HostsModule >() {
				...
				public void handle(
					...
					try {
						definitions = module.hostManagerReference.get().getAliasDefinitions();
						if ( null == pattern ) {
							...
							if ( request.getFlag( "restrict" ) ) {
								for ( SoftwareAliasDefinition definition : definitions ) {
									if ( cpattern.matcher( definition.getAliasName() ).matches() ) {
										writer.sendLineXtend( definition );
									}
								}
							} else {
		...
		actions.put(
			...
			new CommandLineAction< HostsModule >() {
				...
				public void handle(
					...
					try {
						alias = HostDataConvertor.aliasToAlias(
							module.hostManagerReference.get().getAliasDefinitionByName( name )
						);
					} catch ( ValueNotFoundException exception ) {
						throw new ModuleSpecificException(
							Errors.UNKN_ALIAS,
					...
					try {
						...
						throw new ModuleSpecificException( Errors.FAIL_MARSHAL, exception );
					} finally {
						try {
			...
			new CommandLineAction< HostsModule >() {
				...
				public void handle(
					...
					try {
						...
						throw new ModuleSpecificException( Errors.FAIL_UMSL_VAL, exception );
					} catch ( JAXBException exception ) {
						throw new ModuleSpecificException( Errors.FAIL_UMSL, exception );
					} catch ( ConvertorException exception ) {
						throw new ModuleSpecificException( Errors.FAIL_UMSL_CONV, exception );
					}
					...
					try {
						...
						hostManager = module.hostManagerReference.get();
						if ( null == edit ) {
							hostManager.addAliasDefinition(											// !!! Duplicates tolerated !!!
								HostDataConvertor.aliasToAlias( alias )
							);
						} else {
							SoftwareAliasDefinition original;
							try {
								original = hostManager.removeAliasDefinitionByName( edit );
							} catch ( ValueNotFoundException exception ) {
								throw new ModuleSpecificException(
									Errors.UNKN_ALIAS,
									" (" + edit + ')',
							...
							hostManager.addAliasDefinition( original );								// !!! Duplicates tolerated !!!
						}
						hostManager.rebuildAliasTableForAllHosts();
					} catch ( ComponentInitializationException exception ) {
						throw new ModuleSpecificException( Errors.CONN_HM, exception );
					} catch ( HostDatabaseException exception ) {
						throw new ModuleSpecificException( Errors.FAIL_ALIAS_QUERY, exception );
					} catch ( RemoteException exception ) {
						throw new ModuleSpecificException( Errors.CONN_HM, exception );
					}
				}
		...
		actions.put(
			...
			new CommandLineAction< HostsModule >() {
				...
				public void handle(
					...
					try {
						module.hostManagerReference.get().removeAliasDefinitionByName( name );
					} catch ( HostDatabaseException exception ) {
						throw new ModuleSpecificException( Errors.FAIL_ALIAS_QUERY, exception );
					} catch ( ValueNotFoundException exception ) {
						throw new ModuleSpecificException(
					...
					try {
						...
						if ( null == pattern ) {
							...
							if ( null == host ) {
								if ( request.getFlag( "desc" ) ) {
									for ( HostGroup group : groups ) {
										if ( cpattern.matcher( group.getName() ).matches() ) {
											writer.sendLineXtend( group );
										}
									}						
								} else {
								...
								if ( request.getFlag( "desc" ) ) {
									for ( HostGroup group : groups ) {
										if (
											cpattern.matcher( group.getName() ).matches() &&
											group.getAllHosts().contains( host )
										) {
											writer.sendLineXtend( group );
										}
									}
		...
		actions.put(
			...
			new CommandLineAction< HostsModule >() {
				...
				public void handle(
					...
					name = request.getParameter( "name" );
					try {
						group = HostDataConvertor.groupToGroup(
							module.hostManagerReference.get().getGroup( name )
						);
					} catch ( ValueNotFoundException exception ) {
						throw new ModuleSpecificException(
							Errors.UNKN_GROUP,
			...
			new CommandLineAction< HostsModule >() {
				...
				public void handle(
					...
					try {
						hostManager = module.hostManagerReference.get();
						if ( null == edit ) {
							hostManager.addGroup(
								HostDataConvertor.groupToGroup( group, hostManager )
							);
						} else {
							HostGroup original;
							...
							HostDataConvertor.modifyGroup( group, original, hostManager );
							hostManager.addGroup( original );
						}
					} catch ( InvalidArgumentException exception ) {
						throw new ModuleSpecificException( Errors.MALF_GROUP, exception );
					} catch ( ComponentInitializationException exception ) {
						throw new ModuleSpecificException( Errors.CONN_HM, exception );
					} catch ( HostDatabaseException exception ) {
						throw new ModuleSpecificException( Errors.FAIL_GROUP_QUERY, exception );
		...
		actions.put(
			"property-list",
			new CommandLineAction< HostsModule >() {
				private final Set< String > allParameters = stringSet( "pattern", "host" );
				private final Set< String > requiredParameters = stringSet( "host" );
						...
						if ( null == pattern ) {
							if ( request.getFlag( "desc" ) ) {
								...
								if ( request.getFlag( "user" ) ) {
									throw new ModuleSpecificException( Errors.EXCL_USER_DESC );
								} else {
									for ( NameValuePair property : hi.getProperties() ) {
										writer.sendPropDescr( property, 0, table, "" );
									}
									for ( PropertyTreeReadInterface obj : hi.getObjects() ) {
										writer.sendObjDescr( obj, 0, table );
									}							
								}
							} else {
								if ( request.getFlag( "user" ) ) {
									for ( NameValuePair property : hi.getUserProperties() ) {
							...
							if ( request.getFlag( "desc" ) ) {
								...
								if ( request.getFlag( "user" ) ) {
									...
									for ( NameValuePair property : hi.getProperties() ) {
										if ( cpattern.matcher( property.getName() ).matches() ) {
											writer.sendPropDescr( property, 0, table, "" );
										}
									}
									for ( PropertyTreeReadInterface obj : hi.getObjects() ) {
										if ( cpattern.matcher( obj.getName( true ) ).matches() ) {
											writer.sendObjFilDs( obj, 0, table, cpattern );
										}
									}					
								}
							} else {
		...
		actions.put(
			...
			new CommandLineAction< HostsModule >() {
				...
				public void handle(
					...
					try {
						properties = HostDataConvertor.objectToProperties(
							request.getFlag( "user" ) ?
								module.hostManagerReference.get().getHostInfo( host )
								.getUserPropertiesObject() :
									module.hostManagerReference.get().getHostInfo( host )
							);
					} catch ( ValueNotFoundException exception ) {
						throw new ModuleSpecificException(
		...
		actions.put(
			...
			new CommandLineAction< HostsModule >() {
				...
				public void handle(
					...
					errors = verifyArguments( request, parameters, flags, parameters, true );
					if ( null != errors ) {
						throw new ModuleSpecificException( Errors.INVD_PARAMS, errors );
					}
					...
					try {
						hostManager = module.hostManagerReference.get();
						hi = hostManager.getHostInfo( host );
						properties = module.propertiesParser.parse(
					...
					HostDataConvertor.modifyProperties( properties, hi.getUserPropertiesObject() );	// OK, this ca throw. :-)
					
					try {
						hostManager.updateUserProperties( hi );
					} catch ( HostDatabaseException exception ) {
						throw new ModuleSpecificException( Errors.FAIL_HOST_QUERY, exception );
					} catch ( InvalidArgumentException exception ) {								// Host removed on the fly.
						throw new ModuleSpecificException(											// As if it had never existed.
							Errors.UNKN_HOST,
							" (" + host + ')',
							exception
						);
	 ...
	 * 
	 * @throws CommandLineException When JAXB marshallers or unmarshallers cannot be created.
	 */
	public HostsModule() throws CommandLineException {
		...
		try {
			...
			this.propertiesComposer = XSD.PROPERTIES.createComposer( Properties.class );
			this.propertiesParser = XSD.PROPERTIES.createParser( Properties.class );
