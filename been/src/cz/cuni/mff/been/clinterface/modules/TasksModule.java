...
/**
 * A command line interface component that corresponds to the tasks listing screen.
 * 
...
public final class TasksModule extends CommandLineModule {
	...
	public enum Errors implements Message {
		...
		/** Context id or task id invalid. Hard to say due to *bad* error reporting in TM. */
		...
		REQD_ID_CTX_PATH( "Neither ('id', 'context'), nor 'context', nor 'path' specified." ),
		
		/** The 'recursive' flag is required to kill the whole context. */
		REQD_RECURSIVE( "To delete contexts or nodes, 'recursive' must be set." ),
		
		/** A 'real' or 'artificial' IOException occured. TM has *bad* error reporting. */
		FAIL_IO_REMOTE( "A network or I/O error in TaskManager occured." ),
		
		/** Could not parse the XML TaskDescriptor for some reason. Parser errors? Who knows? */
		FAIL_TD_PARSER( "Failed to parse the XML task descriptor." ),
		
		/** IllegalArgumentException from the TM. Nobody knows what exactly happened. */
		MALF_TD_RUNNING( "Task Manager refused the Task Descriptor. Details follow." ),
		...
		/** When the TM throws an IllegalArgumentException, which could mean just about whatever. */
		STAT_RUNNING( "Task refused by Task Manager. It may be running. See logs for details." ),
		...
		INTG_REF( "Integrity error. Could not call a (String) constructor via reflection." ),
	...
	static {
		...
		actions.put(
			"context-list",
			new CommandLineAction< TasksModule >() {
					...
					try {
						if ( null == pattern ) {
							if ( null == id ) {
								...
								contexts = module.taskManagerReference.get().getContexts();
								...
								tasks = module.taskManagerReference.get().getTasksInContext( id );
							...
							if ( null == id ) {
								...
								if ( request.getFlag( "desc" ) ) {
									for ( ContextEntry context : contexts ) {
										if (
											cpattern.matcher( context.getContextName() ).matches()
										) {
											writer.sendLineDescr( context );
										}
									}
		...
		actions.put(
			"context-new",
			new CommandLineAction< TasksModule >() {
				private final Set< String > allParameters = stringSet( "id", "name", "desc" );
		...
		actions.put(
			...
			new CommandLineAction< TasksModule >() {
				...
				public void handle(
					...
					try {
						query = module.taskManagerReference.get().getTaskTreeQuery();
		...
		actions.put(
			"info",
			new CommandLineAction< TasksModule >() {
				private final Set< String > parameters = stringSet( "path", "id", "context" );
				private final Set< String > flags = stringSet( "desc" );
				private final String help = constructHelp( parameters, flags, false );
				...
				public void handle(
					...
					if ( null == path ) {
						if ( null == id || null == context ) {
							...
							try {
								task = module.taskManagerReference.get().getTaskById( id, context );
							} catch ( IllegalArgumentException exception ) {
								throw new ModuleSpecificException(
									Errors.INVD_ID_CTX,
									" (" + id + ", " + context + ')',
						...
						if ( null == id && null == context ) {
							...
							throw new ModuleSpecificException( Errors.EXCL_ID_CTX_PATH );
						}
					}
					writer = new TaskEntryWriter( response );
					try {
					...
					try {
						if ( null == path ) {
							if ( null == id || null == context ) {
								...
								try {
									records = module.taskManagerReference.get().getLogsForTask(
										context,
										id,
										lfrom,
										lto 
							...
							if ( null == id && null == context ) {
								...
								try {
									records = taskManager.getLogsForTask( address, lfrom, lto );
								} catch ( IllegalAddressException exception ) {
									throw new ModuleSpecificException(
										Errors.INVD_ADDRESS,
										" (" + path + ')',
					...
					try {
						if ( null == path ) {
							if ( null == id || null == context ) {
								...
								try {
									handle = module.taskManagerReference.get().getStandardOutput(
										context,
										id
									);
								} catch ( LogStorageException exception ) {
				...
				public void handle(
					...
					if ( null == path ) {
						if ( null == id || null == context ) {
							...
							try {
								handle = module.taskManagerReference.get().getErrorOutput(
									context,
									id
								);
							} catch ( LogStorageException exception ) {
				...
				public void handle(
					...
					try {
						if ( null == path ) {
							if ( null == id || null == context ) {
								...
								try {
									task = module.taskManagerReference.get().getTaskById(
										id,
										context
									);
								} catch ( IllegalArgumentException exception ) {
					...
					try {
						module.taskDescriptorComposer.compose(
							request.getFlag( "resolved" ) ?
								task.getModifiedTaskDescriptor() :									// Must be VALID. Cannot...
								task.getOriginalTaskDescriptor(),									// ...have both RSL and asTask.
							stream
						);
		...
		actions.put(
			...
			new CommandLineAction< TasksModule >() {
				...
				public void handle(
					...
					errors = verifyArguments( request, parameters, flags, true );					// WOW! We want a blob!
					if ( null != errors ) {
						throw new ModuleSpecificException( Errors.INVD_PARAMS, errors );
					}
					...
					try {
						td = module.taskDescriptorParser.parse( request.getBlobStream() );
						...
						throw new ModuleSpecificException( Errors.MALF_TD_RUNNING, exception );
					...
					try {
						if ( null == path ) {														// No path => Expect a context.
							if ( null == context ) {												// No context => Invalid request.
								throw new ModuleSpecificException( Errors.REQD_ID_CTX_PATH );
							} else {																// OK, context specified.
								if ( null == id ) {													// No id, kill the whole context.
									if ( request.getFlag( "recursive" ) ) {							// We require this safety word...
										if ( request.getFlag( "delete" ) ) {						// Delete context when killing?
											module.
											taskManagerReference.get().killAndDeleteContext( context );	// TODO: NOT REFLECTED in the tree.
										} else {													// Don't delete, just kill.
											module.
											taskManagerReference.get().killContextById( context );
										}
									...
									if ( request.getFlag( "delete" ) ) {							// Delete the task after killing?
										taskManager = module.taskManagerReference.get();
										try {
											taskManager.deleteTaskByAddress(
												taskManager.getTaskById( id, context )
												.getTreeAddress()
											);
										} catch ( IllegalAddressException exception ) {
											throw new ModuleSpecificException(
										...
										module.
										taskManagerReference.get().killTaskById( id, context );
									}
								}
							}
						} else {																	// Path specified.
							if ( null == id && null == context ) {
								...
								try {
									...
									throw new ModuleSpecificException(
										Errors.MALF_ADDRESS, " (" + path + ')', exception
									);
								}

								try {
									if ( request.getFlag( "delete" ) ) {							// Delete tasks and nodes?
										if ( request.getFlag( "recursive" ) ) {						// Kill recursively.
											taskManager.deleteNodeByAddress( address );
										} else {													// Expect a leaf, kill one task.
											taskManager.deleteTaskByAddress( address );
										}
									} else {														// Don't delete, just kill?
										if ( request.getFlag( "recursive" ) ) {						// Kill recursively.
						...
						throw new ModuleSpecificException(
							Errors.INVD_ID_CTX, " (" + id + ", " + context + ')', exception
						);							
					} catch ( ComponentInitializationException exception ) {
						throw new ModuleSpecificException( Errors.CONN_TM, exception );
					} catch ( RemoteException exception ) {
	...
	public TasksModule() throws CommandLineException {
		...
		try {
			this.taskDescriptorComposer = XSD.TD.createComposer( TaskDescriptor.class );
