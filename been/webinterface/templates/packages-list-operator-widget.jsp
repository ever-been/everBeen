...
%>
<select name="operator[<%=conditionId%>]">
	<% for (int i = 0; i < operators.length; i++) {%>
		<option
			 value="<%=Routines.htmlspecialchars(operators[i].getName())%>"
			 <%=operators[i].getName().equals(operator) ? " selected=\"selected\"" : ""%>
		><%=Routines.htmlspecialchars(operators[i].getTitle())%></option>
	<% } %>
</select>