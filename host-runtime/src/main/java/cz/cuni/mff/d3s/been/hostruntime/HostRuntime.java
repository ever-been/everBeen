...
final class HostRuntime implements IClusterService {
	...
	private void tryRunTask(RunTaskMessage message) {}

	private void killTask(KillTaskMessage message) {}
	...
	//					FileUtils.copyURLToFile(new URL(baseAddress + "bpk/" + dependency.getBpkId() + "/" + dependency.getGroupId() + "/" + dependency.getVersion()), dependencyFile);
	//					downloadedDependencies.add(dependencyFile);
	//				} catch (IOException e) {
	//					// should rethrow or return on first error
	//					// TODO Auto-generated catch block
	//					e.printStackTrace();
	//				}
	//			}
