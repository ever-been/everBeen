 ...
 * @author David Majda
 */
public class HostRuntimeImplementation extends UnicastRemoteObject
		implements HostRuntimeInterface {
	...
	/** Debug port that will be assigned to first task run by hostruntime (when in debug mode) */
	...
	/**
	 * Returns an unused port that can be used for new task's debugging
	 * and increments port counter 
	 * @return port number 
	 */
	...
	public HostRuntimeImplementation(String taskManagerHostname,
		...
		try {
			taskManagerRuntimePort = (HostRuntimesPortInterface) Naming.lookup(
			...
			public void run() {
				...
				synchronized (runningTasksSync) {
					for (TaskInterface task : runningTasksSync) {
						try { task.kill (); }
						catch (RemoteException e) { throw new AssertionError ("Unexpected remote exception on local call."); }
					}
				}
				
				try { loadMonitor.terminate(); }
				catch (RemoteException e) { throw new AssertionError ("Unexpected remote exception on local call."); }
				...
				} catch (IllegalArgumentException e) {  // TODO: remove this when possible!
	...
	/**
	 * @see cz.cuni.mff.been.hostruntime.HostRuntimeInterface#initialize(HostRuntimesPortInterface, long)
	 */
	public void initialize(HostRuntimesPortInterface hostRuntimesPort, long maxPackageCacheSize) throws RemoteException {
	 ...
	 * 
	 * Note that for this method to make sense, it should be called from a block that already
	 * synchronizes on the list of running tasks, otherwise a new task that does detailed
	 * load monitoring can appear just after this method returns.
	 */
	private boolean runningTaskDoesDetailedMonitoring () {
		synchronized (runningTasksSync)
		{
			for (TaskInterface task: runningTasksSync)
			{
				try { if (task.isDetailedLoad()) return (true); }
				catch (RemoteException e) { throw new AssertionError ("Unexpected remote exception in local call."); }
			}
		}
		return (false);
	}
	...
	public TaskInterface createTask(TaskDescriptor taskDescriptor)
		 ...
		 * TODO Maybe we should be more clever here and not try to locate the reference every time... 
		 */ 
		...
		try
		{
			...
			// The other one is making sure only one running task does detailed load monitoring.
			synchronized (runningTasksSync)
			{
				boolean wantsDetailedLoad = taskDescriptor.isSetLoadMonitoring () ? taskDescriptor.getLoadMonitoring ().isDetailedLoad () : false; 
		...
		} catch (TaskException e) { throw new HostRuntimeException(e); }
	...
	public void deleteContext(String contextID) throws HostRuntimeException, RemoteException {
		...
		if (!new File(contextDir).exists()) {
			throw new IllegalArgumentException ("No task from context \"" + contextID + "\" was run on this Host Runtime.");
		}
		...
		synchronized (runningTasksSync) {
			for (TaskInterface task : runningTasksSync) {
				if (task.getContextID().equals(contextID)) {
					throw new IllegalArgumentException("Task \"" + task.getTaskID() + "\" from context \"" + task.getContextID() + "\" is still running.");
				}
			}
		}
		...
		try {
			Delete.deleteDirectory(rootDirectory
				+ File.separator + TASKS_BASE_DIR
				+ File.separator + contextID);
		} catch (AntTaskException e) { throw new HostRuntimeException(e); }
