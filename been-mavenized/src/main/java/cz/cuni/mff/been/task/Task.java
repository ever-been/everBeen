...
public abstract class Task {
	...
	/** Property name: working directory of a task, it IS NOT deleted immediately after task's end*/
	public static final String PROP_DIR_WORK = "hostruntime.directory.working";
	/** Property name: temporary directory of a task, it IS deleted immediately after task's end */
    ...
    private static Task taskHandle = null;
	...
	/** Directory specified by Host Runtime which contains the task package's content */
	private String taskDirectory = System.getProperty(Task.PROP_DIR_TASK);
	/** Working directory specified by Host Runtime for this task, it IS NOT deleted
	 * immediately after task's end
	 */
	private String workingDirectory = System.getProperty(Task.PROP_DIR_WORK);
	/** Temporary directory for this task specified by Host Runtime, it IS deleted
	 * immediately after task's end
	 */
	private String tempDirectory = System.getProperty(Task.PROP_DIR_TEMP);
	...
	private Logger logger = Logger.getLogger("task." + this.getClass().getCanonicalName());

    /** Pluggable module manager of task. Can be used for loading pluggable modules */
    private PluggableModuleManager pluggableModuleManager;
        
	protected Task() throws TaskInitializationException {
		taskHandle = this;
	 ...
	 */
	public PluggableModule getPluggableModule(String name, String version) throws TaskException, PluggableModuleException {
		return getPluggableModuleManager().getModule(new PluggableModuleDescriptor(name, version));
	}
	
	/** 
	 * Shortcut method for getting pluggable modules easily.
	 * No need to typecast result when using this method.
	 * @param name name of pluggable module
	 * @param version version of pluggable module
	 * @return pluggable module of given name and version
	 * @throws PluggableModuleException 
	 */
	public <T> T getPluggableModule(Class<T> clazz, String name, String version) throws TaskException, PluggableModuleException {
	...
	private void initializePluggableModuleManager() throws TaskException {
		try {
			...
			pluggableModuleManager = new PluggableModuleManagerImpl(tasksPort,
					modulesDir);
		} catch (Exception e) {
			logInfo(e.toString());
			e.printStackTrace();
