...
/**
 * Simple generic implementation of DRBPlanner. Provides 
 * planning that can be suitable for most benchmarks 
 * that have download-build-run workflow.
 * 
 * SimpleDownloadBuildRunPlanner behavior can 
 * but customized by setting some of its properties.
 * 
...
public class SimpleDownloadBuildRunPlanner implements DBRPlanner {
	...
	/**
	 * run / build ratio. Planner will try to keep this ratio when generating new activities
	 */
	...
	/**
	 * run count limit (not more than this number of runs will be performed for every build)
	 */
	private int runLimit;

	/**
	 * set of blacklisted versions. No activities with these versions will be generated.
	 */
	 ...
	 * @param suites set of suites
	 * @param workloadUnits number of computation units that planner can spend
	 * @param buildLimit build count limit
	 * @param runLimit run count limit (for every build)
	 */
	public SimpleDownloadBuildRunPlanner(Set<String> suites, int workloadUnits, int downloadCost, int buildCost,
				int runCost,
			float runBuildRatio, 
	...
	/* (non-Javadoc)
	 * @see cz.cuni.mff.been.benchmarkmanagerng.module.generator.xampler.planner.DBRPlanner#getDownloadActivities()
	 */
	...
	/* (non-Javadoc)
	 * @see cz.cuni.mff.been.benchmarkmanagerng.module.generator.xampler.planner.DBRPlanner#getBuildActivities()
	 */
	...
	/* (non-Javadoc)
	 * @see cz.cuni.mff.been.benchmarkmanagerng.module.generator.xampler.planner.DBRPlanner#getRunActivities()
	 */
	...
	/**
	 * Creates an execution plan (download, build and run activities) 
	 * and stores it in this object's properties.
	 * 
	 * This planner generates activities according to this rules:
	 *  1. if there are some not-yet-downloaded external sources they'll be downloaded first
	 *  2. if there are some binaries that can be run (according to run count limit), run them.
	 *  3. if there are some downloaded sources that can be run (according to build count limit), build them.
	 * 
	 * If build count limit and run count limit for every build are reached
	 * no more activities will be generated.   
	 * 
	 * Planner only makes plan with specified unit cost. If unit number 
	 * becomes zero or less, no more activities will be added into plan.
	 * 
	 ...
	 */
	public void plan(Set<SourceKey> externalSources, Set<SourceKey> sources,
		...
		MultiMap<BuildKey, BuildProps> allBuilds = mergeBuilds(buildLog, availableBinaries);	
		...
		/* builds are planned according to already planned builds, available binaries and available sources*/
		LinkedList<BuildKey> buildQueue = buildQueue(sources, allBuilds);

		/* run queue is made based on already planned runs and available binaries */
		...
		for (SourceKey sourceKey : downloadQueue) {
			if (!versionBlacklist.contains(sourceKey)) {
				...
				logInfo("New download-build-run workflow for version " + sourceKey.getVersion() +" would have been scheduled but the version is BLACKLISTED.");
			}
		...
		while(units > 0) {
			logInfo("Currently having " + buildCountSum + " builds and " + runCountSum + " runs, desired run-build ratio is " + runBuildRatio);
			...
			if ( (tryScheduleRun || buildQueue.isEmpty()) && !runQueue.isEmpty()) {
				...
				if (versionBlacklist.contains(runKey.getVersion())) {
					logInfo("New run workflow for build " + runKey.getVersion() +"/"+runKey.getBuildNumber() + " would have been scheduled but the version is BLACKLISTED.");
					logInfo("Remaining workload units: " + units);
					continue;
				}
				
				RunActivity activity = createRunActivity(runKey, runLog.get(runKey));
				addActivity(activity);
			...
			if (!buildQueue.isEmpty()) {
				...
				if (versionBlacklist.contains(buildKey.getVersion())) {
					logInfo("New build-run workflow for version " + buildKey.getVersion() + " would have been scheduled but the version is BLACKLISTED.");
					logInfo("Remaining workload units: " + units);
					continue;
				}
				
				BuildActivity activity = createBuildActivity(buildKey, allBuilds.get(buildKey));
				addActivity(activity);
			...
			if (buildQueue.isEmpty() && runQueue.isEmpty()) {
				logInfo("No more runs and builds could be scheduled. " + units + " workload units not used.");
				break;
			}
		}		
	...
	/**
	 * Merges two build multi maps together so that every key's 
	 * values are merged. One key will contain only distinct values.
	 * @param buildLog
	...
	private MultiMap<BuildKey, BuildProps> mergeBuilds(
		...
		for (BuildKey key : union) {
			Collection<BuildProps> availableBinariesProps = availableBinaries.get(key);
	...
	private void addActivity(RunActivity activity) {
		...
		logInfo("New run workflow for build " + activity.getVersion() +"/"+activity.getBuildNumber() + " scheduled (new run added).");
	}
	...
	private void addActivity(BuildActivity activity) {
		...
		units -= calculateActivityCost(activity);
		logInfo("New build-run workflow for version " + activity.getVersion() + " scheduled (new build and run added).");
	}
	...
	private void addActivity(DownloadActivity activity) {
		...
		units -= calculateActivityCost(activity);
		logInfo("New download-build-run for version " + activity.getVersion() +" scheduled (new dowload, build and run added).");
	}
	

	/**
	 * 
	 * @param activity  activity for cost evaluation
	 ...
	 * @param finishedRuns list of finished runs
	 * @return created run activity
	 */
	private RunActivity createRunActivity(RunKey runKey, Collection<RunProps> finishedRuns) {
		...
		RunActivity result = new RunActivity(version, buildNumber, runNumber, new TreeSet<String> (suites));		
		return result;
	}

	/**
	 * Creates build activity with dependent run activities
	 * and sets it's build number to MAX(build number) + 1   
	 * @param buildKey build key
	 * @param finishedBuilds list of finished builds 
	 * @return created build activity
	 */
	private BuildActivity createBuildActivity(BuildKey buildKey,
		...
		BuildActivity result = new BuildActivity(version,
				buildNumber);
	 ...
	 * @param sourceKey build key
	 * @return download activity
	 */
	private DownloadActivity createDownloadActivity(
			SourceKey sourceKey) {

		String version = sourceKey.getVersion();

		DownloadActivity result = new DownloadActivity(
				version);
	 ...
	 * 
	 * @param cvsSources external sources
	 * @param sources already downloaded sources
	 * @return queue of download activities sorted by priority
	 */
	private LinkedList<SourceKey> downloadQueue(Set<SourceKey> cvsSources,
	 ...
	 * @param allBuilds already planned builds and already available binaries
	 * 
	 * @return build queue
	 */
	private LinkedList<BuildKey> buildQueue(Set<SourceKey> sources,
		...
		for (BuildKey key : allBuilds.keySet()) {
			...
			if (maxBuildCount < length) maxBuildCount = length;
		}
		...
		for (int i = 0; i <= maxBuildCount; i++) {
			...
			if (builds != null) {
				for (BuildKey buildKey : builds) {
					Collection<BuildProps> buildsOfVersion = allBuilds.get(buildKey);
					if (buildsOfVersion == null) {
						toSort.add(buildKey);
					} else
					if (buildsOfVersion.size() < buildLimit) {
						toSort.add(buildKey);
					}
				}
			}
	 ...
	 * 
	 * @param availableBinaries available binaries 
	 * @param runLog already planned runs
	 * @return index that shows which source version was run how many times
	 */
	private LinkedList<RunKey> runQueue(MultiMap<BuildKey, BuildProps> availableBinaries,
			MultiMap<RunKey, RunProps> runLog) {
		...
		for (BuildKey buildKey : availableBinaries.keySet()) {
			...
			for (BuildProps buildProps : availableBinaries.get(buildKey)) {
				RunKey runKey = new RunKey(buildKey.getVersion(), buildProps
						.getBuildNumber());
		...
		for (RunKey key : runLog.keySet()) {
			...
			if (maxRunCount < length) maxRunCount = length;
		}
	...
	/**
	 * Sorts run keys by version in descending order
	 * and secondarily by build number in ascending order 
	 * @param toSort sort
	 */
	private void sortRunKeys(List<RunKey> toSort) {
		...
		Collections.sort(toSort, new Comparator<RunKey>() {
			...
			public int compare(RunKey o1, RunKey o2) {
				...
				if (res == 0) {
					res = Integer.valueOf(o1.getBuildNumber()).compareTo( o2.getBuildNumber()); 
				}
				return res; 
	...
	private void logInfo(String message) {
		if (Task.getTaskHandle() == null) {
			...
			Task.getTaskHandle().logInfo(message);
		}
	}
