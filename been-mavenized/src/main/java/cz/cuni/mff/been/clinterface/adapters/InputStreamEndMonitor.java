...
/**
 * Monitors the status of an input stream and signals a sleeping thread when the stream is closed.
 * 
 * @author Andrej Podzimek
 */
public final class InputStreamEndMonitor extends RemoteInputStreamMonitor {
	 ...
	 */
	public InputStreamEndMonitor() {
		this.status = RemoteStreamStatus.WORKING;
	}
	
	@Override
	public synchronized void closed( RemoteInputStreamServer stream, boolean clean ) {
		this.status = clean ? RemoteStreamStatus.CLOSED : RemoteStreamStatus.DIRTY;
		notify();
	}
	
	@Override
	public synchronized void failure( RemoteInputStreamServer stream, Exception exception ) {
	 ...
	 */
	public RemoteStreamStatus getStatus() {
		return status;
	}
