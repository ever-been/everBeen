 ...
 *
 */
public class DebugAssistantImplementation extends UnicastRemoteObject implements DebugAssistantInterface {

    /**
     * registered tasks
     */
	private LinkedList<SuspendedTask> suspendedTasks = new LinkedList<SuspendedTask>();
    ...
    @Override
	public synchronized List<SuspendedTask> getSuspendedTasks() throws RemoteException {
	 ...
	 */
	@Override
	public synchronized void registerSuspendedTask(SuspendedTask task) throws RemoteException {
		
		Task.getTaskHandle().logInfo("New suspended task registered: "+ task );
	 ...
	 * @param id suspended task UUID.
	 */
	@Override
	public synchronized void runSuspendedTask(UUID id) throws RemoteException, DebugAssistantException {
		...
		if (suspendedTask == null) {
			throw new DebugAssistantException("Suspended task with given UUID not found.");
		}
		...
		try {
			...
			Task.getTaskHandle().logInfo("Suspended task "+ suspendedTask +" successfully enabled.");
		} catch (IOException e) {
			throw new DebugAssistantException("Error enabling task on host " + host + ":" + port + " to run.", e);
	 ...
	 */
	private void letVMRun(String hostname, int port) throws IOException, IllegalConnectorArgumentsException {
	...
	public synchronized void unregisterTask(UUID id) throws RemoteException,
		...
		Task.getTaskHandle().logInfo("Suspended task "+ suspendedTask +" unregistered from debug assistant.");
